...
export default function Home() {
  ...
  const saveRecord = () => {
    ...
    let next = existing ? (_materialEditAt || autoReopen ? withTrackedUpdate(existing, preparedForSave) : { ...preparedForSave, updateDate: existing.updateDate, lastModifiedAt: existing.lastModifiedAt, _updateHistory: existing._updateHistory }) : preparedForSave;
    ...
    setEditing(null);
    if (!existing) setNewCaseReminder({ ...next });
    flash(autoReopen ? "已重新上架，並記錄在案名後方備註" : "物件已儲存");
  };
  const saveNewCaseReminder = () => {
    if (!newCaseReminder) return;
    const savedAt = new Date().toISOString();
    setRecords(previous => previous.map(record => record.id === newCaseReminder.id ? {
      ...record,
      housingListingCompleted: newCaseReminder.housingListingCompleted || "",
      newBookCompleted: newCaseReminder.newBookCompleted || "",
      wangReviewCompleted: newCaseReminder.wangReviewCompleted || "",
  ...
  return <main lang="en-GB" className={internalView ? "internal-public-app" : ""}>
    {!internalView && <header className="topbar">
      <div className="brand"><h1>總表　管理模式 <small className="app-version">V90</small></h1></div>
...
<div className="modal-foot"><div className="archive-in-editor">{records.some(r => r.id === editing.id) && (editing.archived || isExpired(editing) || editing.status !== "委託中") ? <button type="button" className="restore-in-editor" onClick={() => { setRestoreChoiceRecord(editing); setEditing(null); }}>恢復</button> : records.some(r => r.id === editing.id) && <select defaultValue="" onChange={e => { if (e.target.value) { requestArchive(editing, e.target.value); setEditing(null); } }}><option value="">封存案件…</option><option>售出下架</option><option>成交下架</option><option>下架洽開發</option><option>到期下架</option></select>}</div><div className="record-bottom-actions">{records.some(r => r.id === editing.id) && !editing.archived && editing.status === "委託中" && <><button type="button" onClick={() => returnToIntake(editing)}>退回進案草稿</button><button type="button" className="danger" onClick={() => removeRecord(editing)}>刪除</button></>}</div><div className="record-save-actions">{editing._intakeDraftId && <span className="draft-save-note">尚未進案～進案草稿中</span>}<button type="button" onClick={temporarilySaveRecord}>暫存</button><button className="primary" onClick={saveRecord}>{editing._intakeDraftId ? "同步更新草稿" : "儲存物件"}</button></div></div></div></div>}
    ...
    {newCaseReminder && <div className="modal-backdrop new-case-reminder-backdrop" onMouseDown={event => event.target === event.currentTarget && setNewCaseReminder(null)}><div className="modal new-case-reminder-modal"><div className="modal-head"><div><span>新增案件</span><h2>新案件完成提醒</h2></div><button className="close" type="button" onClick={() => setNewCaseReminder(null)}>×</button></div><div className="new-case-reminder-body"><dl className="new-case-reminder-meta"><div><dt>案名</dt><dd>{newCaseReminder.caseName || "未填寫"}</dd></div><div><dt>地址</dt><dd>{newCaseReminder.address || "未填寫"}</dd></div><div><dt>開發</dt><dd>{developerFullNameText(newCaseReminder.developer || "", settings.personnel) || "未填寫"}</dd></div></dl>{([['housingListingCompleted', '房管上架完成'], ['newBookCompleted', 'NEW本製作完成'], ['wangReviewCompleted', '王總審核完成']] as [string, string][]).map(([key, label]) => <label className="new-case-reminder-check" key={key}><input type="checkbox" checked={newCaseReminder[key] === "1"} onChange={event => setNewCaseReminder(previous => previous ? { ...previous, [key]: event.target.checked ? "1" : "" } : previous)}/><span>{label}</span></label>)}</div><div className="modal-foot"><button type="button" onClick={() => setNewCaseReminder(null)}>稍後處理</button><button type="button" className="primary" onClick={saveNewCaseReminder}>儲存提醒</button></div></div></div>}
  ...
  return <section className="content intake-page">
    ...
    {draft && <>
      <div className="modal-backdrop intake-modal-backdrop" onMouseDown={event => event.target === event.currentTarget && selectDraft("")}><div className="modal intake-draft-modal">
        ...
        <div className="intake-edit-grid">{intakeEditFields.filter(([key]) => !isLandIntakeDraft(draft) || !landHiddenIntakeFields.has(key)).map(([key, label]) => <label className={intakeFieldClass(key)} key={key}><span className={key === "特色說明1" ? "intake-feature-title" : ""}>{label}{key === "特色說明1" && <button type="button" className="copy-features-button" onClick={copyFeatures}>{featuresCopied ? "已複製" : "複製特色1～4"}</button>}</span><input value={intakeValue(draft.values, key)} onChange={e => updateValue(key, e.target.value)}/></label>)}</div>
...
function CellContent({ record: r, column: k }: { record: RecordItem; column: string }) {
  ...
  if (k === "bookLocation") { const rocDate = displayRocDate(r.bookLocationDate || ""); const shortDate = rocDate ? rocDate.split("/").slice(-2).join("/") : ""; const location = r.bookLocationType || ""; return <span className="two-line-value book-location-value"><b>{shortDate || "—"}</b><small>{location || "—"}</small>{r.bookDownDate && <small className="archive-down-note">{displayRocDate(r.bookDownDate)}下架</small>}</span>; }
...
function BusinessInventory({ records, people }: { records: RecordItem[]; people: Person[] }) {
  ...
  return <section className="content business-inventory-page">
    ...
    <div className="inventory-columns"><div className="inventory-panel monthly-inventory-panel" id="monthly-inventory-image">
      ...
      <small>本月件數＝本月進案＋本月重新上架＋本月下架；本月下架只計算本月進案後又在同月下架的案件</small>
    </div>
