...
export default function Home() {
  ...
  return <main lang="en-GB" className={internalView ? "internal-public-app" : ""}>
    {!internalView && <header className="topbar">
      <div className="brand"><h1>總表　管理模式 <small className="app-version">V2</small></h1></div>
...
function CellContent({ record: r, column: k }: { record: RecordItem; column: string }) {
  ...
  if (k === "bookLocation") { const rocDate = displayRocDate(r.bookLocationDate || ""); const shortDate = rocDate ? rocDate.split("/").slice(-2).join("/") : ""; const location = r.bookLocationType || ""; return <span className="book-location-inline">{shortDate || location ? `${shortDate}${location}` : "—"}{r.bookDownDate && <small className="archive-down-note">{displayRocDate(r.bookDownDate)}下架</small>}</span>; }
