...
import { coordinatesFromInput, createColorWorkbookMap, placeColorWorkbookMap } from "./colorWorkbookMap";
import { colorWorkbookLayoutMatches, colorWorkbookPhotoMatches, matchColorWorkbookLayout, matchColorWorkbookPhoto, placeColorWorkbookImage } from "./colorWorkbookImages";
...
type Person = { id: string; sequence?: string; name: string; nationalId: string; phone?: string; lineUrl?: string; role?: "業務" | "秘書"; status: "在職" | "離職" };
...
const PPT_ORDER_RULE_VERSION = "linked-developer-v2";
...
const COLOR_PHOTO_PATH_KEY = "property-desk-color-photo-path-v1";
...
const filesFromColorFolder = async (root: RememberedDirectoryHandle, record: RecordItem, kind: "photo" | "layout") => {
  const files: File[] = [];
  const compactPath = (value: unknown) => String(value || "").normalize("NFKC").toLowerCase().replace(/[\s　_－—–\-｜|·・,，、。.．\/\\()（）\[\]【】]/g, "");
  const needles = [record.caseName, record.address].map(compactPath).filter(Boolean);
  const matches = (path: string) => needles.some(needle => compactPath(path).includes(needle));
  const visit = async (folder: RememberedDirectoryHandle, prefix: string, matchedParent = false) => {
    for await (const entry of folder.values()) {
      const relativePath = `${prefix}${entry.name}`;
      const matched = matchedParent || matches(relativePath);
      if (entry.kind === "directory") {
        // 照片庫很大，只進入案名或完整地址相符的案件資料夾。
        if (matched) await visit(entry as RememberedDirectoryHandle, `${relativePath}/`, true);
      } else if (matched && /\.(?:jpe?g|png)$/i.test(entry.name) && (kind === "layout" || /^1\.(?:jpe?g|png)$/i.test(entry.name))) {
...
const fields = [
  ...
  ["zoningDocumentStatus", "使用分區文件"], ["zoningApplicationDate", "使用分區申請日期"], ["zoningCertificateCount", "已附分區證明（張）"],
  ["authorizationStatus", "授權書狀態"], ["authorizationCopyType", "授權書本別"], ["authorizationCopyCount", "影本（張）"], ["authorizationOriginalCount", "正本（張）"],
...
const recordEditOrder = [
  ...
  "landTitleCount", "buildingTitleCount", "titleUndertaking", "zoningDocumentStatus", "zoningApplicationDate", "zoningCertificateCount", "authorizationStatus", "authorizationCopyCount", "authorizationOriginalCount",
  "websiteHeader",
...
const dateKeys = new Set(["entrustStart", "entrustEnd", "reportDate", "updateDate", "groupViewDate", "showingFollowUpDueDate", "bookLocationDate", "salesBookDate", "coverChangeDate", "zoningApplicationDate", "platform591Expiry", "price5168Expiry", "goldExposureExpiry", "archived"]);
const selectOptions: Record<string, string[]> = {
  status: ["委託中", "售出下架", "租出下架", "成交下架", "下架洽開發"],
...
const parseAreaPaste = (raw: string, record: RecordItem): RecordItem => {
  ...
  return {
    ...
    // 房管「土地持分面積」僅供比對，不可覆蓋業務貼串原始的正式地坪。
    landSharePing: landShare || record.landSharePing || ""
  };
};
const integerPing = (value: unknown) => {
  const number = filterNumber(value);
  return Number.isFinite(number) ? Math.trunc(number) : null;
};
const areaPasteDifferences = (raw: string, parsed: RecordItem, original: RecordItem) => {
  const text = String(raw || "");
  const rows = [
    { label: "總建坪", present: /建物總面積\s*[\d,.]+\s*坪/.test(text), before: original.buildingPing, after: parsed.registryBuildingPing },
    { label: "室內坪", present: /(?:主建物|附屬建物)\s*[\d,.]+\s*坪/.test(text), before: original.indoorPing, after: parsed.registryIndoorPing },
    { label: "地坪", present: /土地持分面積\s*[\d,.]+\s*坪/.test(text), before: original.landPing, after: parsed.landSharePing },
  ];
  return rows.flatMap(row => {
    const before = integerPing(row.before), after = integerPing(row.after);
    return row.present && before !== null && after !== null && before !== after
      ? [{ ...row, before, after }]
      : [];
  });
};
...
const dailyChangedLabels = (keys: string[]) => [...new Set(keys.map(key => ({
  area: "地區", caseName: "案名", caseNameNote: "案名後方備註文字", address: "地址", price: "總價", reducedPrice: "總價", direction: "朝向",
...
export default function Home() {
  ...
  const [colorPhotoPath, setColorPhotoPath] = useState(() => typeof window === "undefined" ? DEFAULT_COLOR_PHOTO_PATH : localStorage.getItem(COLOR_PHOTO_PATH_KEY) || DEFAULT_COLOR_PHOTO_PATH);
  ...
  const readRememberedColorFolder = async (handle: RememberedDirectoryHandle, record: RecordItem, kind: "photo" | "layout", setFiles: Dispatch<SetStateAction<File[]>>, setStatus: Dispatch<SetStateAction<string>>) => {
    setStatus(`正在比對 ${handle.name}…`);
    try { const files = await filesFromColorFolder(handle, record, kind); setFiles(files); setStatus(`已完成 ${handle.name}（找到 ${files.length} 張對應圖片）`); }
    catch (error) { console.error(error); setFiles([]); setStatus(`無法讀取 ${handle.name}，請重新授權或選擇資料夾`); }
  };
  const chooseRememberedColorFolder = async (key: "photo" | "layout", record: RecordItem, handleRef: { current: RememberedDirectoryHandle | undefined }, setFiles: Dispatch<SetStateAction<File[]>>, setStatus: Dispatch<SetStateAction<string>>) => {
    try {
      ...
      // 手動按鈕一定重新開啟選擇視窗；只有自動載入才沿用已記住的資料夾。
      const handle = await picker.call(window, { mode: "read" });
      handleRef.current = handle;
      await saveColorFolderHandle(key, handle);
      await readRememberedColorFolder(handle, record, key, setFiles, setStatus);
    } catch (error) {
  ...
  useEffect(() => {
    const restore = async (key: string, handleRef: { current: RememberedDirectoryHandle | undefined }, setFiles: Dispatch<SetStateAction<File[]>>, setStatus: Dispatch<SetStateAction<string>>) => {
      try {
        ...
        if (!handle) return;
        handleRef.current = handle;
        if (await handle.queryPermission({ mode: "read" }) === "granted") setStatus(`已記住 ${handle.name}，開啟彩色表時自動比對`);
  ...
  useEffect(() => {
    if (!printEditor) return;
    const matchSavedFolder = async (kind: "photo" | "layout", handle: RememberedDirectoryHandle | undefined, setFiles: Dispatch<SetStateAction<File[]>>, setStatus: Dispatch<SetStateAction<string>>) => {
      if (!handle) return;
      if (await handle.queryPermission({ mode: "read" }) !== "granted") return setStatus(`已記住 ${handle.name}，Edge 需要重新授權`);
      await readRememberedColorFolder(handle, printEditor.data, kind, setFiles, setStatus);
    };
    void matchSavedFolder("photo", colorPhotoFolderRef.current, setColorPhotoFiles, setColorPhotoFolderStatus);
    void matchSavedFolder("layout", colorLayoutFolderRef.current, setColorLayoutFiles, setColorLayoutFolderStatus);
  }, [printEditor?.data.id, printEditor?.data.caseName, printEditor?.data.address]);
  useEffect(() => {
    ...
    const coordinateLabel = coordinateField?.querySelector<HTMLElement>("span");
    const coordinateHelp = coordinateField?.querySelector<HTMLElement>("small");
    if (coordinateLabel) coordinateLabel.textContent = "位置座標（必填）";
    if (coordinateInput) coordinateInput.placeholder = "例如：22.97354854089914, 120.2238506558218";
    if (coordinateHelp) coordinateHelp.textContent = "請貼上「緯度, 經度」；沒有正確座標不能下載 Excel。座標會保存在此案件，下次開啟仍會顯示。";
    const requireValidCoordinatesBeforeDownload = (event: MouseEvent) => {
      ...
      if (coordinatesFromInput(printEditor.data.locationMapInput)) {
        field?.classList.remove("missing-coordinate");
        field?.querySelector<HTMLInputElement>("input")?.removeAttribute("aria-invalid");
        return;
      }
      event.preventDefault();
      event.stopPropagation();
      field?.classList.add("missing-coordinate");
      const input = field?.querySelector<HTMLInputElement>("input");
      input?.setAttribute("aria-invalid", "true");
      input?.focus();
    };
    document.addEventListener("click", requireValidCoordinatesBeforeDownload, true);
    return () => document.removeEventListener("click", requireValidCoordinatesBeforeDownload, true);
  }, [printEditor]);
  ...
  const [intakeDeletedIds, setIntakeDeletedIds] = useState<string[]>([]);
  ...
  const [pptDeferredOpen, setPptDeferredOpen] = useState(false);
  ...
  const pendingIntakeReminderRecords = records.filter(record => isIntakeReminderRecord(record) && newCaseReminderPending(record));
  ...
  useEffect(() => { if (!pptPickerOpen) setPptDeferredOpen(false); }, [pptPickerOpen]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved.orderVersion !== PPT_ORDER_RULE_VERSION) {
        saved.orderIds = [];
        saved.orderVersion = PPT_ORDER_RULE_VERSION;
        all[pptWeekStart] = saved;
  ...
  useEffect(() => {
    if (!storageReady || pptWeekStart !== "2026-08-08" || localStorage.getItem("property-desk-ppt-2026-08-17-recover-v2") === "1") return;
    ...
    try {
      const allWeeks = JSON.parse(localStorage.getItem(PPT_WEEK_SELECTIONS_KEY) || "{}");
      const savedWeek = allWeeks[pptWeekStart] || {};
      const nextExtraIds = [...new Set([...(Array.isArray(savedWeek.extraIds) ? savedWeek.extraIds : []), ...recoveredIds])];
      allWeeks[pptWeekStart] = { ...savedWeek, extraIds: nextExtraIds };
      localStorage.setItem(PPT_WEEK_SELECTIONS_KEY, JSON.stringify(allWeeks));
      setPptExtraIds(previous => [...new Set([...previous, ...nextExtraIds])]);
      if (recoveredIds.length === 2) localStorage.setItem("property-desk-ppt-2026-08-17-recover-v2", "1");
    } catch {
      setPptExtraIds(previous => [...new Set([...previous, ...recoveredIds])]);
    }
  ...
  }, [storageReady, pptWeekStart, pptExtraIds]);
  useEffect(() => {
  ...
  useEffect(() => {
    try {
      ...
      const savedIntake = localStorage.getItem(INTAKE_KEY); if (savedIntake) { const saved = JSON.parse(savedIntake); const savedDrafts: IntakeData[] = saved.drafts || (saved.parsed ? [{ ...saved.parsed, raw: saved.raw || "" }] : []); const deletedIds = [...new Set([...(Array.isArray(saved.deletedIds) ? saved.deletedIds.map(String) : []), ...savedDrafts.filter(isLegacyTestIntakeDraft).map(draft => draft.id)])]; const drafts = reconcileIntakeDraftLinks(savedDrafts.filter(draft => !deletedIds.includes(draft.id)), loadedRecords); if (!localStorage.getItem(PHOTO_INTAKE_CLEANUP_KEY)) { const legacyPhotoValues = new Map(drafts.filter(draft => draft.linkedRecordId).map(draft => [draft.linkedRecordId!, new Set(Object.entries(draft.values).filter(([key, value]) => value && (key.includes("進案文件") || key.includes("當下進案文件"))).map(([, value]) => value.trim()))])); loadedRecords = loadedRecords.map(record => { const values = legacyPhotoValues.get(record.id); const current = String(record.photoInfo || "").split(/[／/]/).map(value => value.trim()).filter(Boolean); return values && current.length && current.every(value => values.has(value)) ? { ...record, photoInfo: "" } : record; }); localStorage.setItem(PHOTO_INTAKE_CLEANUP_KEY, "1"); } setIntakeDeletedIds(deletedIds); setIntakeDrafts(drafts); setSelectedIntakeId(saved.selectedId && !deletedIds.includes(saved.selectedId) ? saved.selectedId : drafts.find(draft => !draft.linkedRecordId)?.id || ""); setIntakeRaw(saved.raw || ""); }
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved.date === today() && saved.personId) {
        ...
        else { localStorage.removeItem("case-file-public-daily-login"); setPublicAuthReady(true); }
      } else {
        localStorage.removeItem("case-file-public-daily-login");
        setPublicAuthReady(true);
      }
    } catch { localStorage.removeItem("case-file-public-daily-login"); setPublicAuthReady(true); }
  }, [internalView, publicUnlocked, settings.personnel]);
  useEffect(() => {
    if (!internalView || !publicUnlocked) return;
  ...
  useEffect(() => {
    ...
    const targetedRepairKey = "property-desk-tour-draft-targeted-repair-v412";
    const targetedRepairPending = localStorage.getItem(targetedRepairKey) !== "1";
    const knownTourDraftGhosts = new Set(["EG0540838", "EG0540827"]);
    const knownGhostDrafts = targetedRepairPending ? intakeDrafts.filter(draft => knownTourDraftGhosts.has(intakeValue(draft.values, "委託主約編號").trim())) : [];
    const ghostRecords = records.filter(record => (targetedRepairPending && knownTourDraftGhosts.has(String(record.propertyNo || "").trim())) || (record._intakeDraftId && (record.status === "尚未進案" || record._notEntered === "1")));
    if (targetedRepairPending && knownGhostDrafts.length && !ghostRecords.length && !knownGhostDrafts.some(draft => draft.linkedRecordId || draft.enteredAt)) {
      // 兩筆已在待處理草稿，修復到此完成；之後使用者按「進案」不可再被退回。
      localStorage.setItem(targetedRepairKey, "1");
      return;
    }
    ...
    setIntakeDrafts(previous => {
      ...
      const existingNos = new Set(repaired.map(draft => intakeValue(draft.values, "委託主約編號").trim()));
    ...
    if (targetedRepairPending && (knownGhostDrafts.length || ghostRecords.some(record => knownTourDraftGhosts.has(String(record.propertyNo || "").trim())))) localStorage.setItem(targetedRepairKey, "1");
  ...
  useEffect(() => { if (!internalView && tab !== "public" && storageReady) localStorage.setItem(INTAKE_KEY, JSON.stringify({ raw: intakeRaw, drafts: intakeDrafts, deletedIds: intakeDeletedIds, selectedId: selectedIntakeId })); }, [intakeRaw, intakeDrafts, intakeDeletedIds, selectedIntakeId, storageReady, internalView, tab]);
  ...
  const cloudUploadStatus = !cloudSession?.accessToken ? "未登入" : cloudUploadState === "failed" ? "上傳失敗" : cloudUploadState === "uploading" ? "上傳中" : cloudUploadState === "complete" ? "上傳完成" : localCloudChangesPending ? "待同步" : "上傳完成";
  ...
  // 當期週報標題維持完整的星期六至星期五；實際進案名單只抓到今天。
  const selectedPptIntakeWeek = {
    ...selectedPptWeek,
    end: selectedPptWeek.start <= today() && today() < selectedPptWeek.end ? today() : selectedPptWeek.end,
  };
  ...
  const weeklyPptRecords = sortPptRecords([
    ...records.filter(record => {
      ...
      return (belongsToPptWeek(record, selectedPptIntakeWeek) || manuallyAdded || required) && (manuallyAdded || required || !excludedFromPptWeek(record, selectedPptWeek.start));
    }),
    ...pptDraftRecords.filter(record => pptExtraIds.includes(record.id) || isRequiredAug17PptRecord(record)),
    ...pptAdHocRecords,
  ]);
  const deferredPptRecords = sortPptRecords(records.filter(record => belongsToPptWeek(record, selectedPptIntakeWeek) && excludedFromPptWeek(record, selectedPptWeek.start) && !pptExtraIds.includes(record.id)));
  const pptExtraCandidates = [...records.filter(record => !belongsToPptWeek(record, selectedPptIntakeWeek)), ...pptDraftRecords].filter(record => [record.caseName, record.address].join(" ").toLowerCase().includes(pptExtraSearch.trim().toLowerCase()));
  ...
  const loadPptAssetFolder = (event: ChangeEvent<HTMLInputElement>, kind: "gif" | "layout") => {
    const expectedExtension = kind === "gif" ? /\.gif$/i : /\.jpe?g$/i;
    const files = Array.from(event.target.files || []).filter(file => expectedExtension.test(file.name));
    const grouped = new Map<string, { key: string; label: string; gif?: File; layout?: File }>();
    files.forEach(file => {
      const rawStem = file.name.replace(/\.(?:gif|jpe?g)$/i, "");
      const label = rawStem.replace(/^ppt[\s_\-－]*/i, "");
      const key = normalizePptAssetText(rawStem);
      if (!key) return;
      const pair = grouped.get(key) || { key, label };
      pair[kind] = file;
      grouped.set(key, pair);
    });
    setPptAssetPairs(previous => {
      const merged = new Map(previous.map(pair => [pair.key, { ...pair, [kind]: undefined }]));
      grouped.forEach((pair, key) => merged.set(key, { ...(merged.get(key) || { key, label: pair.label }), ...pair }));
      return [...merged.values()].filter(pair => pair.gif || pair.layout);
    });
    event.target.value = "";
    flash(`已讀取 ${files.length} ${kind === "gif" ? "個 GIF" : "張 JPG"}；另一個資料夾的選擇會保留`);
  };
  const readPptImage = (file: File) => new Promise<{ data: string; width: number; height: number }>((resolve, reject) => {
    ...
    reader.onload = () => {
      ...
      image.onerror = () => reject(new Error(`無法讀取圖片尺寸：${file.name}`));
      image.onload = () => resolve({ data, width: image.naturalWidth || 1, height: image.naturalHeight || 1 });
      image.src = data;
    };
    reader.readAsDataURL(file);
  });
  const containPptImage = (image: { width: number; height: number }, x: number, y: number, w: number, h: number) => {
  ...
  useEffect(() => {
    ...
    rows.forEach((row, index) => {
      const record = selectedPptBaseRecords.find(item => item.id === row.dataset.pptRecordId) || selectedPptBaseRecords[index];
      if (!record) return;
  ...
  const removeFromPptWeek = (record: RecordItem) => {
    ...
    if (record._intakeDraftId || record.id.startsWith("ppt-draft-")) {
  ...
  const temporarilySaveRecord = () => {
    ...
    if (next._intakeDraftId) {
      ...
      localStorage.setItem(INTAKE_KEY, JSON.stringify({ raw: intakeRaw, drafts: savedDrafts, deletedIds: intakeDeletedIds, selectedId: selectedIntakeId }));
  ...
  const saveColorWorkbookLocation = (data: RecordItem) => {
    const existing = records.find(record => record.id === data.id);
    if (!existing) return;
    const locationMapInput = String(data.locationMapInput || "").trim();
    if (String(existing.locationMapInput || "").trim() === locationMapInput) return;
    const savedAt = new Date().toISOString();
    setRecords(previous => previous.map(record => record.id === data.id ? { ...record, locationMapInput, lastModifiedAt: savedAt } : record));
    setEditing(previous => previous?.id === data.id ? { ...previous, locationMapInput, lastModifiedAt: savedAt } : previous);
  };
  const archiveRecord = (r: RecordItem, status = "下架洽開發", archiveDate = today(), salesPerson = r.salesPerson || "", reason = r.archiveReason || "") => {
    ...
    setRecords(prev => prev.map(x => x.id === r.id ? { ...x, status, archived: normalizeDateInput(archiveDate) || today(), salesPerson: salesPerson.trim(), archiveReason: status === "下架洽開發" ? reason.trim() : "", _archiveActionDate: today(), _archiveStatus: status, ...Object.fromEntries(cleanupKeys.map(key => [key, ""])) } : x)); setArchiveChoice(null); flash("已移至封存，並建立下架待辦");
  };
  const completeArchiveCleanup = (record: RecordItem, key: string) => {
    const value = record[key] ? "" : today();
    setRecords(previous => previous.map(item => item.id === record.id ? { ...item, [key]: value } : item));
  ...
  const completeDealTask = (record: RecordItem, key: string) => setRecords(previous => previous.map(item => item.id === record.id ? { ...item, [key]: item[key] ? "" : today() } : item));
  ...
  const restoreRecord = (r: RecordItem, reopened = true) => {
    ...
    const restorePrice = String(r._restorePrice || "").replace(/\s*萬(?:元)?\s*/g, "").trim();
    const changedPrice = reopened && restorePrice && restorePrice !== String(r.reducedPrice || "").trim();
    const modifiedAt = new Date().toISOString();
    setRecords(prev => prev.map(x => x.id === r.id ? {
      ...
      _archiveActionDate: "",
      archiveReason: "",
      entrustEnd: x.entrustEnd < today() ? "" : x.entrustEnd,
      ...(reopened ? {
        _restoredAt: today(),
        caseNameNote: `${shortRocMonthDay(today())}重新上架`,
        caseNameNoteModifiedAt: modifiedAt,
        updateDate: today(),
        lastModifiedAt: modifiedAt,
        ...(changedPrice ? { reducedPrice: restorePrice, reducedPriceModifiedAt: modifiedAt } : {}),
      } : Object.fromEntries(cleanupKeys.map(key => [key, ""])))
    } : x)); setRestoreChoiceRecord(null); flash(reopened ? changedPrice ? `已重新上架並調整為 ${restorePrice} 萬` : "已重新上架" : "已恢復委託中物件");
  };
  ...
  const exportPpt = async () => {
    ...
    for (const record of selected) {
      ...
      const assetPair = pptAssetPairForRecord(record);
      if (assetPair?.gif) {
        const gif = await readPptImage(assetPair.gif).catch(error => { console.error(error); return null; });
        if (gif) slide.addImage({ data: gif.data, ...containPptImage(gif, rightX, 1.0, 3.94, 3.15) });
      }
      if (assetPair?.layout) {
        const layoutImage = await readPptImage(assetPair.layout).catch(error => { console.error(error); return null; });
        if (layoutImage) slide.addImage({ data: layoutImage.data, ...containPptImage(layoutImage, rightX, 4.15, 3.94, 3.15) });
      }
  ...
  const cloudData = () => ({
    ...
    intake: { raw: intakeRaw, drafts: intakeDrafts, deletedIds: intakeDeletedIds },
  ...
  const cloudFetchWithTimeout = async (input: RequestInfo | URL, init?: RequestInit, timeoutMs = 25000) => {
    ...
    try {
      ...
      if (controller.signal.aborted) throw new Error("雲端連線逾時，請確認網路後按「立即重試上傳」");
      throw error;
    } finally {
  ...
  const refreshCloudSession = async (force = false) => {
    ...
    try {
      const res = await cloudFetchWithTimeout(`${settings.supabaseUrl.replace(/\/$/, "")}/auth/v1/token?grant_type=refresh_token`, { method: "POST", headers: { apikey: settings.supabaseKey, "Content-Type": "application/json" }, body: JSON.stringify({ refresh_token: current.refreshToken }) });
  ...
  const supabasePush = async (quiet = false) => {
    ...
    if (!acquireCloudUploadLease()) {
      ...
      setCloudUploadError("另一個總表分頁正在同步；完成後會讀取最新資料");
    ...
    const pushTask = (async () => {
      ...
      try {
        ...
        const checkResponse = await cloudFetchWithTimeout(`${url}?id=eq.${encodeURIComponent(settings.supabaseRecord)}&select=updated_at`, { headers: cloudHeaders(session) });
        ...
        if (res.status === 401) {
          ...
          if (!session) throw new Error("登入已過期，請到設定重新登入雲端帳號");
          res = await cloudFetchWithTimeout(url, { method: "POST", headers: { ...cloudHeaders(session), Prefer: "resolution=merge-duplicates,return=minimal" }, body: payload });
        }
        if (!res.ok) {
  ...
  const supabasePull = async (automatic = false, quiet = false) => {
    ...
    try {
      ...
      if (automatic && data.intake) {
        ...
        const deletedIds = [...new Set([...intakeDeletedIds, ...(Array.isArray(data.intake.deletedIds) ? data.intake.deletedIds.map(String) : []), ...remoteDrafts.filter(isLegacyTestIntakeDraft).map((draft: IntakeData) => draft.id)])];
        setIntakeDeletedIds(deletedIds);
        setIntakeDrafts(previous => reconcileIntakeDraftLinks(mergeIntakeDrafts(previous, remoteDrafts, deletedIds), [...records, ...data.records]));
        setIntakeRaw(previous => previous || data.intake.raw || "");
        setSelectedIntakeId(previous => previous && !deletedIds.includes(previous) ? previous : "");
      }
      ...
      if (automatic || confirm("雲端資料將與本機資料合併，本機已修改但尚未同步的同一筆資料將以雲端版本為準。確定嗎？")) {
        ...
        if (data.intake) {
          ...
          setIntakeRaw(previous => previous || data.intake.raw || "");
          setIntakeDrafts(previous => reconcileIntakeDraftLinks(mergeIntakeDrafts(previous, remoteDrafts, deletedIds), [...records, ...data.records]));
          setSelectedIntakeId(previous => previous && !deletedIds.includes(previous) ? previous : data.intake.selectedId && !deletedIds.includes(data.intake.selectedId) ? data.intake.selectedId : "");
        }
        if (data.tour) {
        ...
        if (data.pptWeeks && typeof data.pptWeeks === "object") {
          try {
            ...
            Object.entries(data.pptWeeks).forEach(([week, value]) => {
              ...
              const requiredExtraIds = week === "2026-08-08"
                ? records.filter(record => ["LA0060477", "LG0128613"].includes(String(record.propertyNo || "").trim())).map(record => record.id)
                : [];
              mergedWeeks[week] = {
                ...localWeek,
                ...cloudWeek,
                // 手動加入週報的案件可能在不同電腦建立；雲端與本機必須取聯集，不能由較舊的一方覆蓋刪除。
                extraIds: [...new Set([...(Array.isArray(localWeek.extraIds) ? localWeek.extraIds : []), ...(Array.isArray(cloudWeek.extraIds) ? cloudWeek.extraIds : []), ...requiredExtraIds])],
  ...
  const unlock = async (requestedId?: unknown, options?: { preserveScope?: boolean; quiet?: boolean; cloudVersion?: string }) => {
    ...
    try {
      ...
      if (!response.ok || !data?.personId || !Array.isArray(data.records)) { setPublicAuthReady(true); return flash("身分證字號錯誤，或人員目前不是在職"); }
  ...
  const confirmIntake = (draftId?: string) => { const targetId = draftId || selectedIntakeRef.current; const target = targetId ? intakeDrafts.find(d => d.id === targetId) : intakeDraft; if (!target) return; const targetNo = intakeValue(target.values, "委託主約編號"); const existing = target.linkedRecordId ? records.find(record => record.id === target.linkedRecordId) : records.find(record => !!targetNo && record.propertyNo === targetNo); const record = intakeToRecord(target, existing); if (!record.propertyNo || !record.caseName) return flash("缺少物件編號或案名，請先確認表單內容"); if (existing) { const firstFormalEntry = !target.enteredAt; const tracked = { ...withTrackedUpdate(existing, record), ...(firstFormalEntry ? { _newCaseReminderEnabled: "1", _newCaseReminderSource: "intake" } : {}) }; const enteredAt = target.enteredAt || new Date().toISOString(); setRecords(prev => prev.map(item => item.id === existing.id ? tracked : item)); setIntakeDrafts(prev => prev.map(draft => draft.id === target.id ? { ...draft, linkedRecordId: existing.id, enteredAt, modifiedAt: enteredAt } : draft)); flash(firstFormalEntry ? "已正式進案；請稍後至「新進案件提醒」完成後續作業" : "已連結並同步更新原總表資料"); return; } if (!confirm(`確定文件已收到，將「${record.caseName}」正式加入總表？`)) return; const enteredAt = new Date().toISOString(); const entered = { ...record, _newCaseReminderEnabled: "1", _newCaseReminderSource: "intake" }; setRecords(prev => [entered, ...prev]); setIntakeDrafts(prev => prev.map(draft => draft.id === target.id ? { ...draft, linkedRecordId: entered.id, enteredAt, modifiedAt: enteredAt } : draft)); flash("已正式進案；請稍後至「新進案件提醒」完成後續作業"); };
  const removeIntakeDraft = (id: string) => { const target = intakeDrafts.find(d => d.id === id); if (!target || !confirm(`確定刪除「${intakeValue(target.values, "案名") || "未命名草稿"}」？`)) return; setIntakeDeletedIds(previous => previous.includes(id) ? previous : [...previous, id]); setIntakeDrafts(prev => prev.filter(d => d.id !== id)); if (selectedIntakeId === id) setSelectedIntakeId(""); };
  ...
  return <main lang="zh-Hant-TW" className={internalView ? `internal-public-app${publicAuthReady ? " public-auth-ready" : ""}` : ""}>
    ...
    {dealCompletionReminderOpen && tab !== "public" && pendingDealCompletion.length > 0 && !expiryReminderOpen && !websitePoReminderOpen && !missingDataReminderOpen && !archiveCleanupReminderOpen && <div className="modal-backdrop archive-cleanup-backdrop"><div className="modal archive-cleanup-modal"><div className="modal-head"><div><span>成交後續提醒</span><h2>有 {pendingDealCompletion.length} 筆成交案件尚未完成後續作業</h2></div><button className="close" onClick={() => setDealCompletionReminderOpen(false)}>×</button></div><div className="archive-cleanup-table-wrap"><table className="archive-cleanup-table deal-completion-table"><thead><tr><th>案件</th><th>開發／銷售</th>{dealCompletionTasks.map(([, label]) => <th key={label}>{label}</th>)}</tr></thead><tbody>{pendingDealCompletion.map(({ record }) => <tr key={record.id}><td className="archive-cleanup-case-cell"><b>案名：{stripRestoredDisplay(record.caseName) || "未命名案件"}</b><small className="archive-cleanup-address">地址：{record.address || "—"}</small><small className="deal-completion-property-no">{record.propertyNo || "—"}</small><small className="deal-completion-status">{displayRocDate(record.archived || "")}成交下架</small></td><td className="deal-completion-people"><span>開發：{developerFullNameText(record.developer, settings.personnel) || "—"}</span><span>銷售：{record.salesPerson || "—"}</span></td>{dealCompletionTasks.map(([key]) => <td key={key}><button className={record[key] ? "archive-cleanup-done" : ""} onClick={() => completeDealTask(record, key)}>{record[key] ? "已完成" : "完成"}</button></td>)}</tr>)}</tbody></table></div><div className="modal-foot"><span>可點選「已完成」恢復為待完成</span><button onClick={() => setDealCompletionReminderOpen(false)}>稍後處理</button></div></div></div>}
...
<div className="modal-foot"><div className="archive-in-editor">{records.some(r => r.id === editing.id) && (editing.archived || isExpired(editing) || editing.status !== "委託中") ? <><button type="button" className="restore-in-editor" onClick={() => { setRestoreChoiceRecord(editing); setEditing(null); }}>恢復</button>{editing.archived && <div className="archive-task-pills">{archiveCleanupTasks(editing).map(task => <button type="button" key={task.key} className={editing[task.key] ? "done" : ""} onClick={() => completeArchiveCleanup(editing, task.key)}>{editing[task.key] ? <><span>{task.label.replace(/下架$/, "")}</span><small>已完成</small></> : task.label}</button>)}</div>}</> : records.some(r => r.id === editing.id) && <select defaultValue="" onChange={e => { if (e.target.value) { requestArchive(editing, e.target.value); setEditing(null); } }}><option value="">封存案件…</option><option>售出下架</option><option>租出下架</option><option>成交下架</option><option>下架洽開發</option><option>到期下架</option></select>}</div><div className="record-bottom-actions">{records.some(r => r.id === editing.id) && !editing.archived && editing.status === "委託中" && <><button type="button" onClick={() => returnToIntake(editing)}>退回進案草稿</button><button type="button" className="danger" onClick={() => removeRecord(editing)}>刪除</button></>}</div><div className="record-save-actions">{editing._intakeDraftId && <span className="draft-save-note">尚未進案～進案草稿中</span>}<button type="button" onClick={temporarilySaveRecord}>暫存</button>{editing._intakeDraftId ? <button className="primary" onClick={() => saveRecord(false)}>同步更新草稿</button> : <><button type="button" className="daily-publish-secondary" onClick={() => saveRecord(true)}>儲存並發布每日動態</button><button className="primary save-only-primary" onClick={() => saveRecord(false)}>僅儲存</button></>}</div></div></div></div>}
    {printEditor && <div className="modal-backdrop print-editor-backdrop"><div className="modal print-editor-modal compact">
      <div className="modal-head print-editor-head"><div><span>彩色銷售資料表</span><h2>下載彩色表 Excel</h2></div><button className="close" type="button" onClick={() => setPrintEditor(null)}>×</button></div>
      <div className="print-editor-body compact-body">
        <section className="print-editor-section basic"><div className="print-editor-compact-grid">
          {[["developer", "開發人員"], ["price", "總價（萬）"], ["key", "KEY編號"], ["currentState", "現況"], ["caseName", "案名"], ["address", "地址"]].map(([key, label]) => <label className={`field print-editor-${key}`} key={key}><span>{label}</span><input type="text" value={printEditor.data[key] || ""} onChange={event => setPrintEditor({ ...printEditor, data: { ...printEditor.data, [key]: event.target.value } })}/></label>)}
          <label className="field print-editor-locationMapInput"><span>{(typeShort(printEditor.data.type) === "土地" || /^(?:LG|LA)/i.test(printEditor.data.propertyNo || "")) ? "土地位置座標（選填）" : "位置圖定位（選填）"}</span><input type="text" value={printEditor.data.locationMapInput || ""} onChange={event => setPrintEditor({ ...printEditor, data: { ...printEditor.data, locationMapInput: event.target.value } })} onBlur={() => saveColorWorkbookLocation(printEditor.data)} placeholder={(typeShort(printEditor.data.type) === "土地" || /^(?:LG|LA)/i.test(printEditor.data.propertyNo || "")) ? "例如：22.97354854089914, 120.2238506558218" : "可貼完整地址，或 22.97354854089914, 120.2238506558218"}/><small>座標會保存在此案件，下次開啟仍會顯示。</small></label>
      ...
      <div className="modal-foot print-editor-foot"><span className="print-editor-warning">重點說明與備註、位置圖定位只套用本次下載；缺件紀錄可另外儲存到案件。</span><button type="button" onClick={() => setPrintEditor(null)}>取消</button><button type="button" onClick={() => saveColorSheetIssue(printEditor.data)}>儲存缺件紀錄</button><button className="primary" type="button" disabled={colorWorkbookDownloading} onClick={async () => {
        ...
        const layoutMatches = colorWorkbookLayoutMatches(colorLayoutFiles, printEditor.data);
        const conflicts = [photoMatches.length > 1 ? `物件照片找到 ${photoMatches.length} 筆對應` : "", layoutMatches.length > 1 ? `格局圖找到 ${layoutMatches.length} 筆對應` : ""].filter(Boolean);
        if (conflicts.length) alert(`案名或地址比對發生衝突：\n${conflicts.join("\n")}\n\n為避免套錯案件，衝突的圖片框會保持空白，不會任選檔案。`);
        let saveHandle: WorkbookSaveHandle | undefined;
        try { saveHandle = await chooseColorWorkbookSaveFile(printEditor.data); }
        catch (error) { if (error instanceof DOMException && error.name === "AbortError") { flash("已取消另存新檔，未下載 Excel"); return; } console.warn("無法開啟另存新檔，改用一般下載：", error); }
        const generalTax = String(printEditor.data.generalLandValueTax || "").trim(); const selfUseTax = String(printEditor.data.selfUseLandValueTax || "").trim(); const taxMessage = `請確認土增稅資料：\n一般增值稅：${generalTax || "未填寫"}\n自用增值稅：${selfUseTax || "未填寫"}\n\n是否確認並下載彩色表 Excel？`; if (!window.confirm(taxMessage)) return;
        saveColorWorkbookLocation(printEditor.data); setColorWorkbookDownloading(true); flash("正在建立彩色表 Excel…"); try { await downloadColorWorkbook(printEditor.data, settings.personnel, matchColorWorkbookPhoto(colorPhotoFiles, printEditor.data), matchColorWorkbookLayout(colorLayoutFiles, printEditor.data), saveHandle); setPrintEditor(null); flash("彩色表 Excel 已下載"); } catch (error) { console.error(error); const reason = error instanceof Error ? error.message : String(error || "未知錯誤"); alert(`彩色表 Excel 下載失敗：${reason}`); flash("彩色表 Excel 下載失敗"); } finally { setColorWorkbookDownloading(false); }
      }}>{colorWorkbookDownloading ? "建立中…" : "下載 Excel"}</button></div>
    </div></div>}
    {pptPickerOpen && <div className="modal-backdrop" onMouseDown={event => event.target === event.currentTarget && setPptPickerOpen(false)}><div className="modal ppt-picker-modal"><div className="modal-head"><div><span>產生 POWERPOINT</span><h2>選擇週報期別與進案物件</h2></div><button className="close" onClick={() => setPptPickerOpen(false)}>×</button></div><div className="ppt-picker-body"><div className="ppt-week-selector"><b>報告週期</b><div className="ppt-week-dropdown"><button type="button" className="ppt-week-current" onClick={() => setPptWeekMenuOpen(value => !value)}><span>{displayPptWeekLabel(selectedPptWeek)}</span><i>{pptWeekMenuOpen ? "▲" : "▼"}</i></button>{pptWeekMenuOpen && <div className="ppt-week-menu">{pptWeekOptions.map(start => { const week = pptWeekOf(start); return <button type="button" className={selectedPptWeek.start === week.start ? "selected" : ""} key={week.start} onClick={() => { setPptWeekStart(week.start); setPptExtraIds([]); setPptWeekMenuOpen(false); }}>{displayPptWeekLabel(week)}</button>; })}</div>}</div></div><div className="ppt-week-range"><b>本週進案</b><span>{displayPptWeekLabel(selectedPptWeek)}</span></div><div className="ppt-meeting-note">進案週期固定為星期六至星期五，下一個星期一報告；歷史週期會持續保留。</div><div className="ppt-case-list"><div className="ppt-case-heading"><span>序</span><span>案名</span><span>地址</span><span>開發</span><span>進案日期</span><span>確認／操作</span></div>{weeklyPptRecords.map((record, index) => <div className="ppt-case-row auto" key={record.id}><strong>{index + 1}</strong><button type="button" className="ppt-case-name-edit" onClick={() => { setPptConfirmedSnapshots(previous => { const next = { ...previous }; delete next[record.id]; return next; }); pptAdHocRecords.some(item => item.id === record.id) ? (setPptAdHocDraft({ ...record }), setPptAdHocOpen(true)) : setEditing(record); }}>{record.caseName || "未命名案件"}</button><span className="ppt-case-address">{chunkText(record.address || "—", 20).map((line, lineIndex) => <span key={`${line}-${lineIndex}`}>{line}</span>)}</span><span>{developerFullNameText(record.developer) || "—"}</span><small className="ppt-case-date">{displayRocDate(record.reportDate) || "—"}{(() => { const assets = pptAssetPairForRecord(record); return <em className={assets?.gif && assets?.layout ? "complete" : assets?.gif || assets?.layout ? "partial" : "missing"}>GIF {assets?.gif ? "✓" : "－"}　JPG {assets?.layout ? "✓" : "－"}</em>; })()}</small><div className="ppt-inline-confirm">{pptConfirmedSnapshots[record.id] ? <button type="button" className="confirmed" onClick={() => setPptConfirmedSnapshots(previous => { const next = { ...previous }; delete next[record.id]; return next; })}>已確認／解除</button> : <button type="button" className="primary" onClick={() => setPptConfirmedSnapshots(previous => ({ ...previous, [record.id]: { ...record, photos: [...(record.photos || [])] } }))}>確認並鎖定</button>}<button className="ppt-remove-pill" onClick={() => removeFromPptWeek(record)}>刪</button></div></div>)}{!weeklyPptRecords.length && <div className="ppt-empty">這個日期區間目前沒有進案物件</div>}</div>{deferredPptRecords.length > 0 && <div className="ppt-deferred-list"><button type="button" className="ppt-deferred-toggle" onClick={() => setPptDeferredOpen(value => !value)}><b>本週已移除／待決定</b><span>{deferredPptRecords.length} 筆　{pptDeferredOpen ? "▲ 收合" : "▼ 展開"}</span></button>{pptDeferredOpen && deferredPptRecords.map(record => { const choice = pptStoredChoices(record._pptWeekChoices)[selectedPptWeek.start] || ""; return <div className="ppt-deferred-row" key={record.id}><div><button type="button" className="ppt-case-name-edit" onClick={() => { setPptConfirmedSnapshots(previous => { const next = { ...previous }; delete next[record.id]; return next; }); pptAdHocRecords.some(item => item.id === record.id) ? (setPptAdHocDraft({ ...record }), setPptAdHocOpen(true)) : setEditing(record); }}>{record.caseName || "未命名案件"}</button><small>{record.propertyNo}　原週期保留</small></div><button className={choice === "next" ? "selected next" : "next"} onClick={() => decideRemovedPpt(record, "next")}>下次報告</button><button className={choice === "skip" ? "selected skip" : "skip"} onClick={() => decideRemovedPpt(record, "skip")}>確定不上PPT</button></div>; })}</div>}{pptAdHocRecords.length > 0 && <div className="ppt-adhoc-added"><h3>本次新增案件</h3>{pptAdHocRecords.map(record => <div key={record.id}><button type="button" className="ppt-case-name-edit" onClick={() => { setPptAdHocDraft({ ...record }); setPptAdHocOpen(true); }}>{record.caseName || "未命名案件"}</button><span>{record.address || "—"}</span><div className="ppt-inline-confirm">{pptConfirmedSnapshots[record.id] ? <button type="button" className="confirmed" onClick={() => setPptConfirmedSnapshots(previous => { const next = { ...previous }; delete next[record.id]; return next; })}>已確認／解除</button> : <button type="button" className="primary" onClick={() => setPptConfirmedSnapshots(previous => ({ ...previous, [record.id]: { ...record, photos: [...(record.photos || [])] } }))}>確認並鎖定</button>}<button type="button" onClick={() => setPptAdHocRecords(previous => previous.filter(item => item.id !== record.id))}>刪</button></div></div>)}</div>}<div className="ppt-add-actions"><label className="ppt-asset-folder-button">選擇 GIF 資料夾<input type="file" accept=".gif,image/gif" multiple {...({ webkitdirectory: "", directory: "" } as any)} onChange={event => loadPptAssetFolder(event, "gif")}/></label><span className="ppt-asset-folder-summary">{pptAssetPairs.some(pair => pair.gif) ? `已讀取 ${pptAssetPairs.filter(pair => pair.gif).length} 個 GIF` : "尚未選擇 GIF 資料夾"}</span><label className="ppt-asset-folder-button">選擇 JPG 資料夾<input type="file" accept=".jpg,.jpeg,image/jpeg" multiple {...({ webkitdirectory: "", directory: "" } as any)} onChange={event => loadPptAssetFolder(event, "layout")}/></label><span className="ppt-asset-folder-summary">{pptAssetPairs.some(pair => pair.layout) ? `已讀取 ${pptAssetPairs.filter(pair => pair.layout).length} 張 JPG` : "尚未選擇 JPG 資料夾"}</span><button type="button" className="ppt-extra-toggle" onClick={() => setPptShowExtras(value => !value)}>＋加入物件</button><button type="button" className="ppt-adhoc-toggle" onClick={() => { setPptAdHocDraft({ ...blankRecord(), id: newId(), status: "尚未進案" }); setPptAdHocOpen(true); }}>＋尚未填寫表單案件</button></div>{pptShowExtras && <><label className="ppt-extra-search">搜尋案名或地址<input type="search" value={pptExtraSearch} onChange={event => setPptExtraSearch(event.target.value)} placeholder="輸入案名或地址"/></label><div className="ppt-extra-list">{pptExtraCandidates.map(record => <label className="ppt-case-row extra" key={record.id}><input type="checkbox" checked={pptExtraIds.includes(record.id)} onChange={event => setPptExtraIds(previous => event.target.checked ? [...previous, record.id] : previous.filter(id => id !== record.id))}/><button type="button" className="ppt-case-name-edit" onClick={() => { setPptConfirmedSnapshots(previous => { const next = { ...previous }; delete next[record.id]; return next; }); pptAdHocRecords.some(item => item.id === record.id) ? (setPptAdHocDraft({ ...record }), setPptAdHocOpen(true)) : setEditing(record); }}>{record.caseName || "未命名案件"}</button><span className="ppt-case-address">{chunkText(record.address || "—", 20).map((line, lineIndex) => <span key={`${line}-${lineIndex}`}>{line}</span>)}</span><span>{developerFullNameText(record.developer) || "—"}</span>{record._intakeDraftId ? <small className="ppt-source-status draft"><b>進案草稿中</b><em>尚未進案</em></small> : record.archived || isExpired(record) || record.status !== "委託中" ? <small className="ppt-source-status archived"><b>封存下架</b><em>{displayRocDate(record.archived || record.entrustEnd || "") || "—"}{record.archived ? (record.status || "下架") : isExpired(record) ? "到期下架" : (record.status || "下架")}</em></small> : <small className="ppt-source-status active">委託中</small>}<span className="ppt-inline-confirm">{pptExtraIds.includes(record.id) ? (pptConfirmedSnapshots[record.id] ? <button type="button" className="confirmed" onClick={event => { event.preventDefault(); setPptConfirmedSnapshots(previous => { const next = { ...previous }; delete next[record.id]; return next; }); }}>已確認／解除</button> : <button type="button" className="primary" onClick={event => { event.preventDefault(); setPptConfirmedSnapshots(previous => ({ ...previous, [record.id]: { ...record, photos: [...(record.photos || [])] } })); }}>確認並鎖定</button>) : null}</span></label>)}{!pptExtraCandidates.length && <div className="ppt-empty">沒有符合的委託中、封存或尚未進案草稿案件</div>}</div></>}</div><div className="modal-foot ppt-picker-foot"><span>本期共 {selectedPptBaseRecords.length} 筆　{pendingPptConfirmCount ? `／待確認 ${pendingPptConfirmCount} 筆` : "／全部已確認"}</span><button onClick={() => setPptPickerOpen(false)}>取消</button><button className="ppt-image-button" onClick={exportPptOrderImage}>產生圖片</button><button className="primary" onClick={exportPpt}>下載 PPT</button></div></div></div>}
...
function ContactDirectory({ people, printable = false }: { people: Person[]; printable?: boolean }) {
  ...
  const lineHref = (value = "") => { const text = String(value || "").trim(); if (!text) return ""; const candidate = /^[a-z][a-z\d+.-]*:\/\//i.test(text) ? text : `https://${text}`; try { const parsed = new URL(candidate); return ["http:", "https:"].includes(parsed.protocol) ? parsed.href : ""; } catch { return ""; } };
  const groupStarts = [1, 12, 23];
  return <section className="contact-directory"><div className="contact-directory-head"><h3>台慶文化崇明店～同仁通訊錄～</h3>{printable && <button type="button" className="contact-print-button" onClick={() => printContactDirectory(people)}>列印通訊錄</button>}</div><div className="contact-table-scroll"><table><thead><tr>{groupStarts.map(start => <Fragment key={start}><th>序</th><th>姓名</th><th>手機</th></Fragment>)}</tr></thead><tbody>{Array.from({ length: 11 }, (_, row) => <tr key={row}>{groupStarts.map(start => { const sequence = start + row; const person = bySequence.get(sequence); const lineUrl = lineHref(person?.lineUrl); return <Fragment key={sequence}><td>{sequence}</td><td><span className="contact-person-name">{person?.name || ""}{lineUrl && <a className="contact-line-link" href={lineUrl} target="_blank" rel="noopener noreferrer" aria-label={`開啟${person?.name || "同仁"}的 LINE`} title="開啟 LINE">LINE</a>}</span></td><td>{person?.phone ? <a href={`tel:${String(person.phone).replace(/\D/g, "")}`}>{displayPhone(person.phone)}</a> : ""}</td></Fragment>; })}</tr>)}</tbody></table></div></section>;
}
...
const isLegacyTestIntakeDraft = (draft: IntakeData) => {
  const propertyNo = intakeValue(draft.values, "委託主約編號").trim().toUpperCase();
  const caseName = intakeValue(draft.values, "案名").trim();
  const address = intakeValue(draft.values, "物件(完整)地址").trim();
  const developer = intakeValue(draft.values, "開發１/開發２").trim();
  const timestamp = intakeValue(draft.values, "時間戳記").trim();
  return propertyNo === "EG123456" || (!draft.linkedRecordId && !propertyNo && !caseName && !address && !developer && !timestamp);
};
const mergeIntakeDrafts = (localDrafts: IntakeData[], cloudDrafts: IntakeData[], deletedIds: Iterable<string> = []) => {
  const merged = new Map<string, IntakeData>();
  const deleted = new Set(deletedIds);
  [...cloudDrafts, ...localDrafts].forEach(draft => {
    if (!draft?.id || deleted.has(draft.id) || isLegacyTestIntakeDraft(draft)) return;
...
function ColorWorkbookAssetPicker({ record, photoPath, setPhotoPath, layoutPath, setLayoutPath, photoFiles, layoutFiles, photoFolderStatus, layoutFolderStatus, choosePhotoFolder, chooseLayoutFolder }: {
  ...
  const fileLabel = (file: File) => String((file as File & { webkitRelativePath?: string }).webkitRelativePath || file.name);
  const status = (files: File[], matches: File[], emptyText: string) => {
    if (!files.length) return <small>{emptyText}</small>;
    if (!matches.length) return <small className="asset-status missing">找不到對應檔案，Excel 此框保持空白。</small>;
...
async function downloadColorWorkbook(record: RecordItem, personnel: Person[] = [], photoFile?: File, layoutFile?: File, saveHandle?: WorkbookSaveHandle) {
  ...
  const coordinateField = document.querySelector<HTMLElement>(".print-editor-locationMapInput");
  coordinateField?.classList.toggle("missing-coordinate", missingLandCoordinate);
  const coordinateInput = coordinateField?.querySelector<HTMLInputElement>("input");
  if (coordinateInput) coordinateInput.setAttribute("aria-invalid", missingLandCoordinate ? "true" : "false");
  if (missingLandCoordinate) coordinateInput?.focus();
  ...
  if (photoFile) {
    try { await placeColorWorkbookImage(zip, photoFile, "photo", isLand, parser, serializer); }
    catch (error) { console.warn("物件照片未嵌入，照片框保持空白：", error); }
  }
  if (layoutFile) {
    try { await placeColorWorkbookImage(zip, layoutFile, "layout", isLand, parser, serializer); }
    catch (error) { console.warn("格局圖未嵌入，格局圖框保持空白：", error); }
  }
  // 位置圖是加值內容。「查詢、產圖、嵌入範本」任一步驟失敗時，
  // 一律保留右側第三個空白框並繼續產生 Excel，不可中止整份下載。
  try {
    if (missingLandCoordinate) throw new Error("土地位置座標未填寫，略過位置圖。 ");
...
async function printKeyLabel(record: RecordItem) {
  ...
  printDocument.open();
  printDocument.write(`<!doctype html><html lang="zh-Hant"><head><meta charset="UTF-8"><title>鑰匙標籤 ${escapeHtml(propertyNo)}</title><style>@page{size:105mm 50mm;margin:0}*{box-sizing:border-box}html,body{width:105mm;height:50mm;margin:0;padding:0;background:#fff;color:#000;font-family:PMingLiU,"新細明體",serif;overflow:hidden}.sheet *{font-family:PMingLiU,"新細明體",serif!important;font-weight:400!important}.sheet{position:relative;width:105mm;height:50mm}.label{position:absolute;left:0;top:0;width:75mm;height:50mm;border:.35mm solid #000}.info{height:25mm;padding:.8mm 1.4mm .6mm;border-bottom:.3mm dashed #555;display:flex;flex-direction:column;justify-content:space-between}.line{display:flex;align-items:center;gap:1.2mm;white-space:nowrap}.top{font-size:10pt}.top b{font-size:11pt}.right{margin-left:auto}.case{font-size:10.5pt;max-width:52mm;overflow:hidden;text-overflow:ellipsis}.developer{margin-left:auto;font-size:9pt}.address{font-size:9.5pt;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.metrics{font-size:8.5pt}.company{font-size:8pt;display:flex;justify-content:space-between}.blank{height:25mm}.side{position:absolute;left:75mm;top:0;width:30mm;height:50mm}.side-top{height:25mm}.side-card{width:30mm;height:12.5mm;border:.35mm solid #000;padding:.6mm;display:flex;align-items:center;justify-content:center;text-align:center;line-height:1.05;overflow:hidden;overflow-wrap:anywhere}.side-case{font-size:9.5pt}.side-address{font-size:8.5pt}</style></head><body><main class="sheet"><section class="label"><div class="info"><div class="line top"><b>${escapeHtml(propertyNo)}</b><span class="right">公司#${escapeHtml(keyNumber)}</span></div><div class="line"><span class="case">${escapeHtml(record.caseName || "—")}</span><span class="developer">${escapeHtml(developerFullNameText(record.developer) || "—")}</span></div><div class="address">${escapeHtml(record.address || "—")}</div><div class="metrics">${escapeHtml(metrics)}</div><div class="company"><span>台南文化崇明加盟店</span><span>電話：06-3356699</span></div></div><div class="blank"></div></section><aside class="side"><div class="side-top"></div><div class="side-card side-case">${escapeHtml(record.caseName || "—")}</div><div class="side-card side-address">${escapeHtml(record.address || "—")}</div></aside></main></body></html>`);
  printDocument.close();
...
function printRecordDocument(record: RecordItem, kind: "color" | "cover") {
  ...
  const hasCoverChange = !!coverChangeNo && !isImportedDashPlaceholder(coverChangeNo);
  const coverNoChange = record.coverNoChange === "無契變" && !hasCoverChange;
  // 契變（ED）列的用途必須完全依照物件編輯「契變 → 用途」欄位，
  // 不可再用底價欄位自行拼接，否則會覆蓋業務實際填寫的用途。
  ...
  const coverContractRows = false ? `<table class="contract-grid-v3"><colgroup><col style="width:25mm"><col style="width:11mm"><col style="width:11mm"><col style="width:11mm"><col style="width:58mm"><col></colgroup><tbody>
      ...
      <tr style="height:12mm"><td>${coverNoChange ? "無契變" : value("coverChangeNo")}</td><td></td><td></td><td></td><td>${coverNoChange ? "-" : record.coverChangeDate ? date("coverChangeDate") : "-"}</td><td>${coverNoChange ? "-" : record.coverChangePurpose ? value("coverChangePurpose") : "-"}</td></tr>
      ...
      <tr style="height:12mm"><td>${coverNoChange ? "無契變" : (isImportedDashPlaceholder(record.coverChangeNo) ? "－" : value("coverChangeNo"))}</td><td>${coverNoChange || isImportedDashPlaceholder(record.coverChangeDate) ? "－" : date("coverChangeDate")}</td><td>${coverNoChange || isImportedDashPlaceholder(record.coverChangePurpose) && !hasCoverChange ? "－" : coverChangePurpose}</td></tr>
  ...
  <div class="contract-title">契約紀錄</div><table class="contract-table"><thead><tr><th>契約編號</th><th>日期／委託起訖日</th><th>用途</th></tr></thead><tbody>
    ...
    <tr><td>${coverNoChange ? "無契變" : value("coverChangeNo")}</td><td>${date("coverChangeDate")}</td><td>${value("coverChangePurpose")}</td></tr>
  </tbody></table>
  ...
  const coverV3 = `<div class="page intake-cover-v3"><style>
    ...
    <div class="document-wrap"><div class="vertical-label document-label"><span>內</span><span>附</span><span>文</span><span>件</span><span>之</span><span>正</span><span>本</span></div><div class="document-content">
      <div class="stars">★主約　<span class="red">${value("propertyNo")}</span>　★契變　<span class="red">${coverNoChange ? "無契變" : (isImportedDashPlaceholder(record.coverChangeNo) ? "－" : value("coverChangeNo"))}</span></div>
...
function DailyActivity({ records, compact = false, onEdit }: { records: RecordItem[]; compact?: boolean; onEdit?: (record: RecordItem) => void }) {
  ...
  const updateRecords = records.filter(record => {
    const changed = dailyUpdateFields(record, selectedDate);
    const restored = dateOnly(record._restoredAt) === selectedDate;
    // 已封存案件若在下架後又修改資料（例如鑰匙改成公司#2），仍要列在「更新物件」。
  ...
  const removedRecords = records.filter(record => !isCommissioned(record) && dateOnly(record._archiveActionDate || record.archived) === selectedDate).map(record => {
    ...
    return { ...record, caseNameNote: `${shortRocMonthDay(record.archived || record._archiveActionDate)}${status}${reason ? `：${reason}` : ""}`, _dailyHighlight: "[]" };
  });
  const groups = [
    { key: "added", title: "新增物件", records: records.filter(record => dateOnly(record._dailyAddedAt || record.reportDate) === selectedDate && dateOnly(record._restoredAt) !== selectedDate) },
...
const intakeDraftEditorTitle = (draft: IntakeData) => {
  const address = intakeValue(draft.values, "物件(完整)地址").trim();
  const addressParts = address.match(/^(?:臺灣省|台灣省)?([^縣市]{1,8}[縣市])(.{1,8}?(?:區|鄉|鎮|市))(.*)$/);
  const city = addressParts?.[1]?.replace(/臺/g, "台") || "";
  const district = addressParts?.[2] || "";
  const area = city && city !== "台南市" && district
    ? `${city.replace(/[縣市]$/, "")}${district.replace(/[區鄉鎮市]$/, "")}`
    : district || "未填地區";
  const districtEnd = district ? address.indexOf(district) + district.length : -1;
  const shortAddress = (districtEnd >= district.length ? address.slice(districtEnd) : addressParts?.[3] || address).trim() || "未填地址";
...
function ChecklistPageV2({ val }: { val: (...keys: string[]) => string }) {
  return <article className="print-page checklist-page">
    ...
    <div className="check-acquire"><b>12. 產權取得年份</b>_____年____月，取得原因：☐買賣　☐法拍　☐繼承　☐贈與　其他：______</div>
...
function PropertyTable({ records, columns, publicMode = false, dailyMode = false, archiveMode = false, activeLead = false, expiryAnnotation = false, showAreaGroups = false, zoom = 100, onEdit, onArchive, onRestore, onRemove, onPptChange }: { records: RecordItem[]; columns: string[]; publicMode?: boolean; dailyMode?: boolean; archiveMode?: boolean; activeLead?: boolean; expiryAnnotation?: boolean; showAreaGroups?: boolean; zoom?: number; onEdit: (r: RecordItem) => void; onArchive: (r: RecordItem, status?: string) => void; onRestore: (r: RecordItem) => void; onRemove: (r: RecordItem) => void; onPptChange?: (r: RecordItem, patch: Partial<RecordItem>) => void }) {
  ...
  const caseNameCell = (r: RecordItem, key: string) => {
  ...
  return <div className={`table-wrap ${publicMode ? "public-table" : ""} ${dailyMode ? "daily-table" : ""} ${activeLead ? "active-lead-table" : ""}`} onScroll={event => publicMode && setPublicScrollLeft(event.currentTarget.scrollLeft)}>
    {publicMode && publicScrollLeft > 80 && hoveredPublicCase && <div className="public-floating-case-name" style={{ left: publicScrollLeft + 8, top: hoveredPublicCaseTop }}>{hoveredPublicCase}</div>}
    <table style={publicMode ? { zoom: zoom / 100 } : undefined}><thead><tr>
      {columns.map((k, i) => <th key={k} className={`${i < stickyCount ? `sticky sticky-${i}` : ""} col-${k}`}><ColumnLabel column={k}/></th>)}
      {showPpt && <><th className="ppt-col">加入PPT</th><th className="ppt-type-col">PPT分類</th></>}
      {!publicMode && records.some(r => r.archived || isExpired(r) || r.status !== "委託中") && <th className="actions-col">操作</th>}
    </tr></thead><tbody>{records.map((r, recordIndex) => {
      const highlighted: string[] = (() => { try { return JSON.parse(r._dailyHighlight || "[]"); } catch { return []; } })();
      const isHighlighted = (key: string) => highlighted.includes(key) || (key === "caseName" && highlighted.includes("caseNameNote")) || (key === "coverageFar" && (highlighted.includes("coverage") || highlighted.includes("far")));
      return <Fragment key={r.id}>
        {showAreaGroups && (recordIndex === 0 || areaGroup(records[recordIndex - 1]) !== areaGroup(r)) && <tr className="area-group-row"><td colSpan={columns.length + (showPpt ? 2 : 0)}><span>{areaGroup(r)}</span></td></tr>}
        <tr onMouseEnter={event => { if (publicMode) { setHoveredPublicCase(r.caseName || r.propertyNo || "未命名案件"); setHoveredPublicCaseTop(event.currentTarget.offsetTop + 6); } }} onMouseLeave={() => publicMode && setHoveredPublicCase("")}>
          {columns.map((k, i) => <td key={k} className={`${i < stickyCount ? `sticky sticky-${i}` : ""} col-${k} ${dateKeys.has(k) && k !== "groupViewDate" && !validDate(r[k]) ? "date-error" : ""} ${dailyMode && isHighlighted(k) ? "daily-updated-cell" : ""}`}>{k === "caseName" ? caseNameCell(r, k) : k === "status" ? <span className={`status ${displayStatus(r) === "委託中" ? "live" : "off"}`}>{displayStatus(r)}</span> : <CellContent record={r} column={k}/>}</td>)}
          {showPpt && <><td className="ppt-col"><input type="checkbox" checked={pptIncluded(r)} onChange={e => onPptChange(r, { pptSelected: e.target.checked ? "1" : "0", pptCategory: r.pptCategory || (isCurrentPptWeek(r.reportDate) ? "本週進案" : "臨時新增") })}/></td><td className="ppt-type-col"><select value={pptCategory(r)} onChange={e => onPptChange(r, { pptCategory: e.target.value, pptSelected: "1" })}><option>本週進案</option><option>臨時新增</option></select></td></>}
          {!publicMode && (r.archived || isExpired(r) || r.status !== "委託中") && <td className="actions-col"><div className="row-actions"><button onClick={() => onRestore(r)}>恢復</button><button className="danger" onClick={() => onRemove(r)}>刪除</button></div></td>}
        </tr>
      </Fragment>;
    })}</tbody></table>
  </div>;
...
function CellContent({ record: r, column: k }: { record: RecordItem; column: string }) {
  ...
  if (k === "parking") {
    ...
    const methodShort: Record<string, string> = { "坡道/平面": "坡平", "坡道/機械": "坡機", "昇降/平面": "昇平", "昇降/機械": "昇機", "獨立車庫": "車庫", "庭院": "庭院", "車庫": "車庫", "平移/機械": "平機" };
  ...
  if (k === "zoning") { const chars = Array.from(r.zoning || ""); const lines = Array.from({ length: Math.ceil(chars.length / 4) }, (_, index) => chars.slice(index * 4, index * 4 + 4).join("")); return lines.length ? <span className="floor-lines zoning-lines">{lines.map((line, index) => <span key={`${line}-${index}`}>{line}</span>)}</span> : <>—</>; }
  ...
  if (k === "bookLocation") { const rocDate = displayRocDate(r.bookLocationDate || ""); const shortDate = rocDate ? rocDate.split("/").slice(-2).join("/") : ""; const location = r.bookLocationType || ""; return <span className="book-location-value"><b>{shortDate || "—"}{location || ""}</b>{!isCommissioned(r) && r.bookDownDate && <small className="archive-down-note">{displayRocDate(r.bookDownDate)}下架</small>}</span>; }
...
function MonthlyPropertyReport({ records, person, submit, holdUntil }: { records: RecordItem[]; person: Person; submit: (entries: Array<{ record: RecordItem; status: string; reason: string }>) => Promise<boolean>; holdUntil: number }) {
  ...
  if (today() < firstConfirmationDate) return null;
  if (!eligibleRecords.length) return <section className="monthly-report-panel monthly-report-complete"><div className="monthly-report-head"><div><b>每45天物件確認</b><span>{person.name}，本次待確認物件已全部送出。</span></div><strong>已完成</strong></div></section>;
...
function CompactGroupsOverview({ records, settings, groups }: { records: RecordItem[]; settings: Settings; groups: InventoryGroup[] }) {
  const [filter, setFilter] = useState<"ALL" | InventoryGroup["id"]>(groups.length > 1 ? "ALL" : groups[0]?.id || "A");
  const [monthlyOpenIds, setMonthlyOpenIds] = useState<string[]>([]);
  const [stockOpenIds, setStockOpenIds] = useState<string[]>([]);
  const [sort, setSort] = useState<{ key: "name" | "role" | "monthly" | "stock"; direction: "asc" | "desc" }>({ key: "monthly", direction: "desc" });
  ...
  const namesFor = (record: RecordItem) => { const names = [...new Set(developerNameLines(String(record.developer || "").replace(/[\s、，,／/+&和與]/g, ""), settings.personnel).filter(name => !secretaryNames.includes(name)))]; return names.length ? names : ["未歸屬業務"]; };
  ...
  const monthlyIds = visibleRows.filter(row => row.monthly > 0).map(row => row.member.personId), stockIds = visibleRows.filter(row => row.stock > 0).map(row => row.member.personId);
  const allMonthlyOpen = monthlyIds.length > 0 && monthlyIds.every(id => monthlyOpenIds.includes(id)), allStockOpen = stockIds.length > 0 && stockIds.every(id => stockOpenIds.includes(id));
  const toggle = (ids: string[], id: string) => ids.includes(id) ? ids.filter(value => value !== id) : [...ids, id];
  const changeSort = (key: typeof sort.key) => setSort(current => ({ key, direction: current.key === key && current.direction === "desc" ? "asc" : "desc" }));
  const arrow = (key: typeof sort.key) => sort.key === key ? sort.direction === "desc" ? "↓" : "↑" : "↕";
  const caseList = (items: RecordItem[], name: string, prefix: string) => <div className="compact-case-list">{items.map(record => ({ record, weight: weightFor(record, name) })).filter(item => item.weight > 0).map(({ record, weight }) => <div key={`${prefix}-${record.id}`}><b>{record.caseName || "未命名案件"}</b><span>{record.address || "—"}</span><strong>{numberText(weight)}件</strong></div>)}</div>;
  return <section className="compact-groups-overview">
    <div className="compact-groups-toolbar"><div className="compact-group-filters">{groups.length > 1 && <button className={filter === "ALL" ? "selected" : ""} onClick={() => setFilter("ALL")}>全部</button>}{groups.map(group => <button className={`${filter === group.id ? "selected" : ""} group-${group.id.toLowerCase()}`} onClick={() => setFilter(group.id)} key={group.id}>{group.name || `${group.id}組`}</button>)}</div><div className="compact-expand-actions"><button className="monthly" onClick={() => setMonthlyOpenIds(allMonthlyOpen ? [] : monthlyIds)}>{allMonthlyOpen ? "收合本月進案明細" : "展開本月進案明細"}</button><button className="stock" onClick={() => setStockOpenIds(allStockOpen ? [] : stockIds)}>{allStockOpen ? "收合庫存明細" : "展開庫存明細"}</button></div></div>
    <div className="compact-groups-table-wrap"><table><colgroup><col className="compact-person-col"/><col className="compact-role-col"/><col className="compact-monthly-col"/><col className="compact-stock-col"/></colgroup><tbody>{visibleGroups.map(group => <Fragment key={group.id}><tr className={`compact-group-heading group-${group.id.toLowerCase()}`}><td><b>{group.name || `${group.id}組`}</b><button onClick={() => changeSort("name")}>人員 <strong>{arrow("name")}</strong></button></td><td><button onClick={() => changeSort("role")}>身分 <strong>{arrow("role")}</strong></button></td><td><button onClick={() => changeSort("monthly")}>本月進案 <strong>{arrow("monthly")}</strong></button></td><td><button onClick={() => changeSort("stock")}>目前庫存 <strong>{arrow("stock")}</strong></button></td></tr>{rowsFor(group).map(row => { const monthlyOpen = monthlyOpenIds.includes(row.member.personId), stockOpen = stockOpenIds.includes(row.member.personId); return <Fragment key={`${group.id}-${row.member.personId}`}><tr className="compact-person-row"><td>{row.name}</td><td>{row.member.role}</td><td><button className="compact-count monthly" disabled={!row.monthly} onClick={() => setMonthlyOpenIds(ids => toggle(ids, row.member.personId))}>{numberText(row.monthly)}</button></td><td><button className="compact-count stock" disabled={!row.stock} onClick={() => setStockOpenIds(ids => toggle(ids, row.member.personId))}>{numberText(row.stock)}</button></td></tr>{(monthlyOpen || stockOpen) && <tr className="compact-detail-row"><td colSpan={2}></td><td className="monthly-detail">{monthlyOpen && <><h4>{row.name}　本月進案明細</h4>{caseList(monthlyRecords, row.name, "monthly")}</>}</td><td className="stock-detail">{stockOpen && <><h4>{row.name}　目前庫存明細</h4>{caseList(activeRecords, row.name, "stock")}</>}</td></tr>}</Fragment>; })}</Fragment>)}</tbody></table></div>
  </section>;
}

function LegacyLeaderTeamOverview({ records, settings, group }: { records: RecordItem[]; settings: Settings; group: InventoryGroup }) {
  ...
  const namesFor = (record: RecordItem) => {
    const parsed = [...new Set(developerNameLines(String(record.developer || "").replace(/[\s、，,／/+&和與]/g, ""), settings.personnel).filter(name => !secretaryNames.includes(name)))];
...
function LeaderTeamOverview({ records, settings, group }: { records: RecordItem[]; settings: Settings; group: InventoryGroup }) {
  ...
  if (group.name === "全部組別") {
    ...
    return <CompactGroupsOverview records={records} settings={settings} groups={separatedGroups}/>;
  }
  return <CompactGroupsOverview records={records} settings={settings} groups={[group]}/>;
  const peopleById = new Map(settings.personnel.map(person => [person.id, person]));
  const secretaryNames = settings.personnel.filter(person => person.role === "秘書").map(person => person.name.trim()).filter(Boolean);
  const namesFor = (record: RecordItem) => {
    const parsed = [...new Set(developerNameLines(String(record.developer || "").replace(/[\s、，,／/+&和與]/g, ""), settings.personnel).filter(name => !secretaryNames.includes(name)))];
  ...
  const sortedRows = [...rows].sort((left, right) => {
    const difference = teamSort.key === "name" ? left.name.localeCompare(right.name, "zh-TW") : teamSort.key === "role" ? left.member.role.localeCompare(right.member.role, "zh-TW") || left.name.localeCompare(right.name, "zh-TW") : left[teamSort.key] - right[teamSort.key];
    return (teamSort.direction === "asc" ? difference : -difference) || left.name.localeCompare(right.name, "zh-TW");
  });
  const changeTeamSort = (key: "name" | "role" | "monthly" | "stock") => setTeamSort(current => ({ key, direction: current.key === key && current.direction === "desc" ? "asc" : "desc" }));
  const monthlyExpandableIds = rows.filter(row => row.monthly > 0).map(row => row.member.personId);
  const stockExpandableIds = rows.filter(row => row.stock > 0).map(row => row.member.personId);
  const allMonthlyExpanded = monthlyExpandableIds.length > 0 && monthlyExpandableIds.every(id => monthlyDetailPersonIds.includes(id));
  const allStockExpanded = stockExpandableIds.length > 0 && stockExpandableIds.every(id => stockDetailPersonIds.includes(id));
  const toggleId = (ids: string[], id: string) => ids.includes(id) ? ids.filter(value => value !== id) : [...ids, id];
  const caseList = (items: RecordItem[], name: string, prefix: string) => <div className="leader-case-list">{detailItems(items, name).map(({ record, weight }) => <div key={`${prefix}-${record.id}`}><span>{record.caseName || "未命名案件"}</span><small>{record.address || "—"}</small><strong>{numberText(weight)}件</strong></div>)}</div>;
  return <section className={`leader-team-overview group-${group.id.toLowerCase()}`}>
    <header><div className="leader-team-head-actions"><button type="button" onClick={() => setMonthlyDetailPersonIds(allMonthlyExpanded ? [] : monthlyExpandableIds)}>{allMonthlyExpanded ? "收合本月進案明細" : "展開本月進案明細"}</button><button type="button" onClick={() => setStockDetailPersonIds(allStockExpanded ? [] : stockExpandableIds)}>{allStockExpanded ? "收合庫存明細" : "展開庫存明細"}</button></div></header>
    <div className="leader-team-summary"><div><b>{rows.length}</b><span>組內人數</span></div><div><b>{numberText(rows.reduce((sum, row) => sum + row.monthly, 0))}</b><span>本月進案</span></div><div><b>{numberText(rows.reduce((sum, row) => sum + row.stock, 0))}</b><span>目前庫存</span></div></div>
    <div className="leader-team-table-wrap"><table><thead><tr><th><button type="button" className="leader-sort-button" onClick={() => changeTeamSort("name")}>人員 <b>{teamSort.key === "name" ? teamSort.direction === "desc" ? "↓" : "↑" : "↕"}</b></button></th><th><button type="button" className="leader-sort-button" onClick={() => changeTeamSort("role")}>身分 <b>{teamSort.key === "role" ? teamSort.direction === "desc" ? "↓" : "↑" : "↕"}</b></button></th><th><button type="button" className="leader-sort-button" onClick={() => changeTeamSort("monthly")}>本月進案 <b>{teamSort.key === "monthly" ? teamSort.direction === "desc" ? "↓" : "↑" : "↕"}</b></button></th><th><button type="button" className="leader-sort-button" onClick={() => changeTeamSort("stock")}>目前庫存 <b>{teamSort.key === "stock" ? teamSort.direction === "desc" ? "↓" : "↑" : "↕"}</b></button></th></tr></thead><tbody>{sortedRows.map(row => {
      const monthlyOpen = monthlyDetailPersonIds.includes(row.member.personId), stockOpen = stockDetailPersonIds.includes(row.member.personId);
      return <Fragment key={row.member.personId}><tr><td>{row.name}</td><td>{row.member.role}</td><td><button type="button" className="leader-detail-button monthly" disabled={!row.monthly} onClick={() => setMonthlyDetailPersonIds(ids => toggleId(ids, row.member.personId))}>{numberText(row.monthly)}</button></td><td><button type="button" className="leader-detail-button stock" disabled={!row.stock} onClick={() => setStockDetailPersonIds(ids => toggleId(ids, row.member.personId))}>{numberText(row.stock)}</button></td></tr>{(monthlyOpen || stockOpen) && <tr className="leader-detail-row combined"><td colSpan={2}></td><td className="leader-monthly-detail-cell">{monthlyOpen && <><b>{row.name}　本月進案明細</b>{caseList(monthlyRecords, row.name, "leader-month")}</>}</td><td className="leader-stock-detail-cell">{stockOpen && <><b>{row.name}　目前庫存明細</b>{caseList(activeRecords, row.name, "leader-stock")}</>}</td></tr>}</Fragment>;
    })}</tbody></table></div>
  </section>;
}
...
function BusinessInventory({ records, settings, setSettings }: { records: RecordItem[]; settings: Settings; setSettings: (settings: Settings) => void }) {
  ...
  const emptyCategory = () => ({ exclusiveHouse: 0, exclusiveLand: 0, generalHouse: 0, generalLand: 0, rental: 0, total: 0 });
  const emptyStockRow = (name: string) => ({ name, ...emptyCategory() });
  const emptyMonthlyRow = (name: string) => ({ name, entered: emptyCategory(), archived: emptyCategory(), total: emptyCategory() });
  const personNameById = (personId: string) => people.find(person => person.id === personId)?.name || "";
  ...
  const addCategories = (items: ReturnType<typeof emptyCategory>[]) => items.reduce((sum, item) => ({ exclusiveHouse: sum.exclusiveHouse + item.exclusiveHouse, exclusiveLand: sum.exclusiveLand + item.exclusiveLand, generalHouse: sum.generalHouse + item.generalHouse, generalLand: sum.generalLand + item.generalLand, rental: sum.rental + item.rental, total: sum.total + item.total }), emptyCategory());
  const groupFilterButtons = <div className="inventory-group-filter"><button className={inventoryGroupFilter === "ALL" ? "selected" : ""} onClick={() => setInventoryGroupFilter("ALL")}>全部</button>{inventoryGroups.map(group => <button className={`${inventoryGroupFilter === group.id ? "selected" : ""} group-${group.id.toLowerCase()}`} onClick={() => setInventoryGroupFilter(group.id)} key={group.id}>{group.name || `${group.id}組`}</button>)}{groupSections.some(section => section.id === "UNGROUPED" && section.members.length > 0) && <button className={inventoryGroupFilter === "UNGROUPED" ? "selected" : ""} onClick={() => setInventoryGroupFilter("UNGROUPED")}>未分組</button>}</div>;
  ...
  const stockItemsFor = (name: string) => activeRecords.filter(record => weightFor(record, name) > 0).sort((a, b) => String(a.propertyNo || "").localeCompare(String(b.propertyNo || ""), "zh-TW")).map(record => ({ record, weight: weightFor(record, name) }));
  ...
  const exportInventoryCanvas = () => {
    const title = `${rocMonth}進案與目前庫存統計`;
    const headers = ["組別", "身分", "人員", "本月進案", "本月下架", "本月件數", "房專", "土專", "房一", "土一", "租件", "庫存總數"];
    const rows = visibleGroupSections.flatMap(section => section.members.map(member => { const monthly = monthlyRows.find(item => item.name === member.name) || emptyMonthlyRow(member.name); const stock = stockRows.find(item => item.name === member.name) || emptyStockRow(member.name); return [section.name, member.role, member.name, categoryText(monthly.entered), categoryText(monthly.archived), categoryText(monthly.total, true), numberText(stock.exclusiveHouse), numberText(stock.exclusiveLand), numberText(stock.generalHouse), numberText(stock.generalLand), numberText(stock.rental), `${numberText(stock.total)}件`]; }));
    const widths = [150, 130, 200, 460, 280, 220, 135, 135, 135, 135, 125, 175]; const margin = 44, titleHeight = 124, headerHeight = 88, rowHeight = 86, tableWidth = widths.reduce((sum, value) => sum + value, 0);
    ...
    let x = margin, y = margin + titleHeight; context.font = "bold 34px Microsoft JhengHei"; headers.forEach((header, index) => { context.fillStyle = index >= 3 && index <= 5 ? "#f8dfc1" : index >= 6 ? "#d8edf0" : "#e7ece9"; context.fillRect(x, y, widths[index], headerHeight); context.strokeStyle = index >= 3 && index <= 5 ? "#d7a66c" : index >= 6 ? "#77aeb6" : "#9fb0a8"; context.lineWidth = 1.8; context.strokeRect(x, y, widths[index], headerHeight); context.fillStyle = index >= 3 && index <= 5 ? "#7c4a18" : index >= 6 ? "#225e67" : "#354b43"; context.fillText(header, x + widths[index] / 2, y + headerHeight / 2); x += widths[index]; });
    const shownRows = rows.length ? rows : [["目前沒有資料", ...headers.slice(1).map(() => "-")]]; shownRows.forEach((row, rowIndex) => { x = margin; y = margin + titleHeight + headerHeight + rowIndex * rowHeight; row.forEach((value, index) => { context.fillStyle = index >= 3 && index <= 5 ? "#fffaf3" : index >= 6 ? index === row.length - 1 ? "#e9f6f7" : "#f5fbfc" : "#ffffff"; context.fillRect(x, y, widths[index], rowHeight); context.strokeStyle = index >= 3 && index <= 5 ? "#e1c5a1" : index >= 6 ? "#afd0d4" : "#bdc9c3"; context.lineWidth = 1.8; context.strokeRect(x, y, widths[index], rowHeight); context.fillStyle = index === row.length - 1 ? "#176775" : index >= 3 && index <= 5 ? "#6d4a28" : "#17242a"; context.font = `${index === 0 || index === row.length - 1 ? "bold " : ""}27px Microsoft JhengHei`; context.fillText(String(value), x + widths[index] / 2, y + rowHeight / 2); x += widths[index]; }); });
    canvas.toBlob(blob => { if (!blob) return alert("圖片產生失敗"); const link = document.createElement("a"); const url = URL.createObjectURL(blob); link.href = url; link.download = `${rocMonth}進案與目前庫存統計.png`; document.body.appendChild(link); link.click(); setTimeout(() => { URL.revokeObjectURL(url); link.remove(); }, 3000); }, "image/png");
  };
  ...
  return <section className="content business-inventory-page">
    ...
    <section className={`inventory-group-settings${groupSettingsOpen ? " open" : " collapsed"}`}><div className="inventory-group-settings-head"><div><h3>A～D 分組人員設定</h3><p>指定每位人員所屬組別及組長；預設收合以節省畫面空間。</p></div><button type="button" className="group-settings-toggle" onClick={() => setGroupSettingsOpen(value => !value)}>{groupSettingsOpen ? "收合設定" : "展開設定分組"}</button></div>{groupSettingsOpen && <div className="inventory-group-grid">{inventoryGroups.map(group => <article className={`inventory-group-card group-${group.id.toLowerCase()}`} key={group.id}><header><b>{group.id}</b><input value={group.name} onChange={event => updateInventoryGroup(group.id, { name: event.target.value })}/><button type="button" onClick={() => addInventoryMember(group.id)}>＋ 新增</button></header><div className="inventory-group-labels"><span>人員</span><span>身分</span><span>專</span><span>一般</span><span>計數</span><span></span></div>{group.members.map((member, index) => { const stock = stockForPerson(member.personId); return <div className="inventory-group-member" key={`${group.id}-${index}-${member.personId}`}><select value={member.personId} onChange={event => changeInventoryMember(group.id, index, event.target.value)}><option value="">選擇人員</option>{activeSalesPeople.map(person => <option value={person.id} key={person.id}>{person.name}</option>)}</select><select value={member.role} onChange={event => changeInventoryRole(group.id, index, event.target.value as InventoryGroupMember["role"])}><option>組長</option><option>組員</option></select><span>{numberText(stock.exclusive)}</span><span>{numberText(stock.general)}</span><strong>{numberText(stock.total)}</strong><button type="button" className="danger" onClick={() => removeInventoryMember(group.id, index)}>×</button></div>})}{!group.members.length && <div className="inventory-group-empty">尚未設定人員</div>}</article>)}</div>}</section>
    <div className="inventory-panel combined-inventory-panel" id="combined-inventory-image">
      <div className="inventory-panel-title"><div><h3>{rocMonth}進案與目前庫存統計</h3><p>本月進案 {numberText(monthlyGrandTotal)} 件　｜　目前庫存 {numberText(stockGrandTotal)} 件</p>{groupFilterButtons}</div><div className="inventory-title-actions"><button onClick={exportInventoryCanvas}>產圖</button><button className="monthly-expand-button" onClick={() => setMonthlyExpanded(value => !value)}>{monthlyExpanded ? "收合本月進案明細" : "展開本月進案明細"}</button><button className="stock-expand-button" onClick={() => setStockExpanded(value => !value)}>{stockExpanded ? "收合目前庫存明細" : "展開目前庫存明細"}</button></div></div>
      <table><colgroup><col className="identity-col"/><col className="identity-col"/><col className="monthly-person-col"/><col className="monthly-stat-col"/><col className="monthly-stat-col"/><col className="monthly-stat-col"/><col className="stock-person-col"/><col className="stock-stat-col"/><col className="stock-stat-col"/><col className="stock-stat-col"/><col className="stock-stat-col"/><col className="stock-stat-col"/><col className="stock-stat-col"/></colgroup><thead><tr><th rowSpan={2}>組別</th><th rowSpan={2}>身分</th><th className="monthly-stat-head" colSpan={4}>{rocMonth}進案統計</th><th className="stock-stat-head" colSpan={7}>目前庫存</th></tr><tr><th className="monthly-stat-subhead">人員</th><th className="monthly-stat-subhead">本月進案</th><th className="monthly-stat-subhead">本月下架</th><th className="monthly-stat-subhead">本月件數</th><th className="stock-stat-subhead">人員</th><th className="stock-stat-subhead">房專</th><th className="stock-stat-subhead">土專</th><th className="stock-stat-subhead">房一</th><th className="stock-stat-subhead">土一</th><th className="stock-stat-subhead">租件</th><th className="stock-stat-subhead">總件數</th></tr></thead>
      <tbody>{visibleGroupSections.map(section => { const rows = section.members.map(member => ({ member, monthly: monthlyRows.find(row => row.name === member.name) || emptyMonthlyRow(member.name), stock: stockRows.find(row => row.name === member.name) || emptyStockRow(member.name) })); const entered = addCategories(rows.map(item => item.monthly.entered)); const archived = addCategories(rows.map(item => item.monthly.archived)); const monthlyTotal = addCategories(rows.map(item => item.monthly.total)); const stockTotal = addCategories(rows.map(item => item.stock)); return <Fragment key={`combined-${section.id}`}><tr className={`inventory-group-heading group-${String(section.id).toLowerCase()}`}><td colSpan={13}><b>{section.name}</b><span>{section.members.length} 人</span></td></tr>{rows.map(({ member, monthly, stock }) => <Fragment key={`${section.id}-${member.name}`}><tr><td><span className={`inventory-group-badge group-${String(section.id).toLowerCase()}`}>{section.name}</span></td><td>{member.role === "組長" ? <strong className="inventory-leader-badge">組長</strong> : <span className="inventory-member-label">組員</span>}</td><td className="monthly-stat-cell person-repeat-cell">{member.name}</td><td className="monthly-stat-cell">{categorySummary(monthly.entered)}</td><td className="monthly-stat-cell">{categorySummary(monthly.archived)}</td><td className="monthly-stat-cell inventory-total"><button className="inventory-count-button" onClick={() => setMonthlyDetailName(member.name)}>{categorySummary(monthly.total, true)}</button></td><td className="stock-stat-cell person-repeat-cell">{member.name}</td><td className="stock-stat-cell">{numberText(stock.exclusiveHouse)}</td><td className="stock-stat-cell">{numberText(stock.exclusiveLand)}</td><td className="stock-stat-cell">{numberText(stock.generalHouse)}</td><td className="stock-stat-cell">{numberText(stock.generalLand)}</td><td className="stock-stat-cell">{numberText(stock.rental)}</td><td className="stock-stat-cell inventory-total"><button className="inventory-count-button" onClick={() => setStockDetailName(member.name)}>{numberText(stock.total)}</button></td></tr>{monthlyExpanded && monthly.total.total > 0 && <tr className="inventory-month-detail-row monthly-expanded-detail"><td colSpan={2}></td><td colSpan={4}><b className="inline-detail-title">{member.name}　本月進案明細</b><div className="inventory-case-list inventory-inline-detail">{monthlyItemsFor(member.name).map(({ record, action, weight }, index) => <div key={`${record.id}-${action}-${index}`}><em>{action}</em>{inventoryCaseLine(record)}<strong>{numberText(weight)}件</strong></div>)}</div></td><td colSpan={7}></td></tr>}{stockExpanded && stock.total > 0 && <tr className="inventory-month-detail-row stock-expanded-detail"><td colSpan={2}></td><td colSpan={4}></td><td colSpan={7}><b className="inline-detail-title">{member.name}　目前庫存明細</b><div className="inventory-case-list inventory-inline-detail">{stockItemsFor(member.name).map(({ record, weight }) => <div key={`stock-${record.id}`}><em>庫存</em>{inventoryCaseLine(record)}<strong>{numberText(weight)}件</strong></div>)}</div></td></tr>}</Fragment>)}<tr className="inventory-group-subtotal"><td colSpan={2}>{section.name}小計</td><td className="monthly-stat-cell">{section.name}</td><td className="monthly-stat-cell">{categorySummary(entered)}</td><td className="monthly-stat-cell">{categorySummary(archived)}</td><td className="monthly-stat-cell">{categorySummary(monthlyTotal, true)}</td><td className="stock-stat-cell">{section.name}</td><td className="stock-stat-cell">{numberText(stockTotal.exclusiveHouse)}</td><td className="stock-stat-cell">{numberText(stockTotal.exclusiveLand)}</td><td className="stock-stat-cell">{numberText(stockTotal.generalHouse)}</td><td className="stock-stat-cell">{numberText(stockTotal.generalLand)}</td><td className="stock-stat-cell">{numberText(stockTotal.rental)}</td><td className="stock-stat-cell">{numberText(stockTotal.total)}</td></tr></Fragment>; })}{!visibleGroupSections.some(section => section.members.length) && <tr><td colSpan={13} className="inventory-empty">目前沒有分組人員</td></tr>}{inventoryGroupFilter === "ALL" && <tr className="inventory-company-total"><td colSpan={5}>全公司總計</td><td>{numberText(monthlyGrandTotal)}件</td><td colSpan={6}></td><td>{numberText(stockGrandTotal)}件</td></tr>}</tbody></table>
...
function Field({ fieldKey, label, record, records, setRecord }: { fieldKey: string; label: string; record: RecordItem; records: RecordItem[]; setRecord: (r: RecordItem) => void }) {
  ...
  if (fieldKey === "areaPaste") return <label className="field area-paste-field"><span>{label}</span><textarea rows={8} value={value} onPaste={event => { const textarea = event.currentTarget; window.setTimeout(() => { const raw = textarea.value; const differences = areaPasteDifferences(raw, parseAreaPaste(raw, record), record); if (differences.length) alert(`房管面積與原本資料不同（只比較整數）：\n${differences.map(item => `${item.label}：原本 ${item.before} 坪／房管 ${item.after} 坪`).join("\n")}`); }, 0); }} onChange={event => setRecord(parseAreaPaste(event.target.value, record))} placeholder="貼上建物面積、土地面積、建築完成日與主要建材整串文字，系統會自動抓取下方數字。"/><small>已抓取的內容仍可在下方個別修改；面積只比較整數，不比較小數點後數字。</small></label>;
  const pillChoices: Record<string, string[]> = {
    ...
    authorizationStatus: ["缺授", "無需要"],
    parkingOwnership: ["無車位", "停自有地", "固定車位", "車位另租", "抽籤決定", "先到先停"], parkingMethod: ["坡道/平面", "坡道/機械", "昇降/平面", "昇降/機械", "庭院", "車庫", "平移/機械"]
  };
  if (fieldKey === "coverNoChange") { const hasChangeNo = !!String(record.coverChangeNo || "").trim() && !isImportedDashPlaceholder(record.coverChangeNo); return <label className={`field cover-pill-field cover-pill-${fieldKey}`}><span>{label}</span><span className="pill-options">{hasChangeNo ? <button type="button" className="active" disabled>有契變</button> : <button type="button" className={value === "無契變" ? "active" : ""} onClick={() => setRecord(value === "無契變" ? { ...record, coverNoChange: "" } : { ...record, coverNoChange: "無契變", coverChangeNo: "", coverChangeDate: "", coverChangePurpose: "" })}>無契變</button>}</span></label>; }
  if (fieldKey === "zoningDocumentStatus") { const options = ["房屋不需要", "謄本已標示不用附"]; return <label className={`field cover-pill-field cover-pill-${fieldKey}`}><span>{label}</span><span className="pill-options">{options.map(option => <button type="button" className={value === option ? "active" : ""} onClick={() => set(value === option ? "" : option)} key={option}>{option}</button>)}</span></label>; }
  ...
  if (["buildingPing", "indoorPing", "landPing", "registryBuildingPing", "registryIndoorPing", "landSharePing"].includes(fieldKey)) { const parsed = record.areaPaste ? parseAreaPaste(record.areaPaste, record) : record; const compared = fieldKey === "buildingPing" ? parsed.registryBuildingPing : fieldKey === "indoorPing" ? parsed.registryIndoorPing : fieldKey === "landPing" ? parsed.landSharePing : fieldKey === "registryBuildingPing" ? record.buildingPing : fieldKey === "registryIndoorPing" ? record.indoorPing : record.landPing; const fromHouseManager = ["registryBuildingPing", "registryIndoorPing", "landSharePing"].includes(fieldKey); const currentInteger = integerPing(value), comparedInteger = integerPing(compared); const mismatch = Boolean(record.areaPaste && currentInteger !== null && comparedInteger !== null && currentInteger !== comparedInteger); const prefix = fromHouseManager ? "進案" : "房管"; return <label className={`field compared-ping-field${mismatch ? " ping-mismatch" : ""}`}><span><b>{label}</b>{compared && <small>{prefix} {compared} 坪{mismatch ? "　⚠ 整數不同" : ""}</small>}</span><input inputMode="decimal" value={value} onChange={event => set(event.target.value)}/></label>; }
  ...
  if (fieldKey === "bookLocationReason") {
    ...
    return <label className="field book-location-reason-field"><span>旁5原因 <b className="required-mark">必填</b></span><input type="text" value={value} onChange={e => set(e.target.value)} placeholder="請填寫原因"/></label>;
  }
  ...
  if (["zoningCertificateCount", "authorizationCopyCount", "authorizationOriginalCount"].includes(fieldKey)) return <label className="field"><span>{label}</span><input type="text" inputMode="numeric" value={value} onChange={event => set(event.target.value.replace(/\D/g, ""))} placeholder="請填張數"/></label>;
  ...
  if (dateKeys.has(fieldKey)) return <label className={`field ${value && !validDate(normalizeDateInput(value)) ? "invalid" : ""}`}><span>{label}</span><input type="text" inputMode="numeric" placeholder="例如 7/24" value={displayRocDate(normalizeDateInput(value))} onChange={e => set(normalizeCompleteDateInput(e.target.value))} onBlur={e => set(normalizeDateInput(e.target.value))}/>{fieldKey !== "zoningApplicationDate" && <small>可輸入 7/24，自動轉為民國日期</small>}</label>;
  return <label className="field"><span>{label}{fieldKey === "propertyNo" || fieldKey === "caseName" ? " *" : ""}</span><input type="text" inputMode={["price", "reducedPrice", "builtYear", "indoorPing", "buildingPing", "landPing", "coverage", "far", "zoningCertificateCount", "authorizationCopyCount", "authorizationOriginalCount"].includes(fieldKey) ? "decimal" : undefined} value={inputValue} onChange={e => set(e.target.value)}/>{fieldKey === "builtYear" && value && <small>目前換算：{ageOf(record)}</small>}</label>;
}

function SettingsPanel({ settings, setSettings, supabasePush, supabasePull, cloudSession, cloudUploadStatus, cloudUploadError, supabaseSignIn, supabaseSignOut }: { settings: Settings; setSettings: (s: Settings) => void; supabasePush: () => void; supabasePull: () => void; cloudSession: CloudSession | null; cloudUploadStatus: string; cloudUploadError: string; supabaseSignIn: (email: string, password: string, signUp?: boolean) => void; supabaseSignOut: () => void }) {
  ...
  const reorderPerson = (id: string, rawSequence: string) => {
    const current = settings.personnel.find(person => person.id === id);
    if (!current) return;
    const sameStatus = sortPeopleBySequence(settings.personnel.filter(person => person.id !== id && person.status === current.status));
    const otherStatus = sortPeopleBySequence(settings.personnel.filter(person => person.status !== current.status));
    const requested = Math.floor(Number(rawSequence));
    const position = Number.isFinite(requested) && requested > 0 ? Math.min(requested, sameStatus.length + 1) : sameStatus.length + 1;
    sameStatus.splice(position - 1, 0, current);
    const ordered = current.status === "在職" ? [...sameStatus, ...otherStatus] : [...otherStatus, ...sameStatus];
    setSettings({ ...settings, personnel: ordered.map((person, index) => ({ ...person, sequence: String(index + 1) })) });
  };
  const personTextInput = (person: Person, field: "name" | "nationalId" | "phone" | "lineUrl", placeholder: string, type = "text") => {
  ...
  const addPerson = () => setSettings({ ...settings, personnel: [...settings.personnel, { id: newId(), sequence: String(settings.personnel.length + 1), name: "", nationalId: "", phone: "", lineUrl: "", role: "業務", status: "在職" }] });
