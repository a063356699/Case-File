...
export default function Home() {
  ...
  return <main lang="en-GB" className={internalView ? `internal-public-app${publicAuthReady ? " public-auth-ready" : ""}` : ""}>
    {!internalView && <header className="topbar">
      <div className="topbar-row"><div className="brand"><h1>總表　管理模式 <small className="app-version">V278</small></h1></div>
...
function BusinessInventory({ records, settings, setSettings }: { records: RecordItem[]; settings: Settings; setSettings: (settings: Settings) => void }) {
  ...
  const exportInventoryCanvas = (kind: "monthly" | "stock") => {
    ...
    const headers = monthly ? ["組別", "身分", "人員", "本月進案", "本月下架", "本月件數"] : ["組別", "身分", "人員", "房專", "土專", "房一", "土一", "租件", "總件數"];
    const rows = monthly
      ? visibleGroupSections.flatMap(section => section.members.map(member => { const row = monthlyRows.find(item => item.name === member.name) || emptyMonthlyRow(member.name); return [section.name, member.role, row.name, categoryText(row.entered), categoryText(row.archived), categoryText(row.total, true)]; }))
      : visibleGroupSections.flatMap(section => section.members.map(member => { const row = stockRows.find(item => item.name === member.name) || emptyStockRow(member.name); return [section.name, member.role, row.name, numberText(row.exclusiveHouse), numberText(row.exclusiveLand), numberText(row.generalHouse), numberText(row.generalLand), numberText(row.rental), `${numberText(row.total)}件`]; }));
    const widths = monthly ? [170, 150, 220, 650, 330, 260] : [170, 150, 220, 170, 170, 170, 170, 150, 200]; const margin = 44, titleHeight = 124, headerHeight = 88, rowHeight = 86, tableWidth = widths.reduce((sum, value) => sum + value, 0);
  ...
  return <section className="content business-inventory-page">
    ...
    <div className="inventory-columns"><div className="inventory-panel monthly-inventory-panel" id="monthly-inventory-image">
      <div className="inventory-panel-title"><div><h3>{rocMonth}進案統計　總件數 :{numberText(monthlyGrandTotal)}件</h3>{groupFilterButtons}</div><div className="inventory-title-actions"><button onClick={() => exportInventoryCanvas("monthly")}>產圖</button><button onClick={() => setMonthlyExpanded(value => !value)}>{monthlyExpanded ? "收合" : "展開"}</button></div></div>
      <table><colgroup><col style={{width:"12%"}}/><col style={{width:"11%"}}/><col style={{width:"15%"}}/><col style={{width:"36%"}}/><col style={{width:"14%"}}/><col style={{width:"12%"}}/></colgroup><thead><tr><th>組別</th><th>身分</th><th>人員</th><th>本月進案</th><th>本月下架</th><th>本月件數</th></tr></thead>
      <tbody>{visibleGroupSections.map(section => { const rows = section.members.map(member => ({ member, row: monthlyRows.find(row => row.name === member.name) || emptyMonthlyRow(member.name) })); const entered = addCategories(rows.map(item => item.row.entered)); const archived = addCategories(rows.map(item => item.row.archived)); const total = addCategories(rows.map(item => item.row.total)); return <Fragment key={`monthly-${section.id}`}><tr className={`inventory-group-heading group-${String(section.id).toLowerCase()}`}><td colSpan={6}><b>{section.name}</b><span>{section.members.length} 人</span></td></tr>{rows.map(({ member, row }) => <Fragment key={`${section.id}-${row.name}`}><tr><td><span className={`inventory-group-badge group-${String(section.id).toLowerCase()}`}>{section.name}</span></td><td>{member.role === "組長" ? <strong className="inventory-leader-badge">組長</strong> : <span className="inventory-member-label">組員</span>}</td><td>{row.name}</td><td>{categorySummary(row.entered)}</td><td>{categorySummary(row.archived)}</td><td className="inventory-total"><button className="inventory-count-button" onClick={() => setMonthlyDetailName(row.name)}>{categorySummary(row.total, true)}</button></td></tr>{monthlyExpanded && row.total.total > 0 && <tr className="inventory-month-detail-row"><td colSpan={6}><div className="inventory-case-list inventory-inline-detail">{monthlyItemsFor(row.name).map(({ record, action, weight }, index) => <div key={`${record.id}-${action}-${index}`}><em>{action}</em>{inventoryCaseLine(record)}<strong>{numberText(weight)}件</strong></div>)}</div></td></tr>}</Fragment>)}<tr className="inventory-group-subtotal"><td colSpan={3}>{section.name}小計</td><td>{categorySummary(entered)}</td><td>{categorySummary(archived)}</td><td>{categorySummary(total, true)}</td></tr></Fragment>; })}{!visibleGroupSections.some(section => section.members.length) && <tr><td colSpan={6} className="inventory-empty">本月目前沒有分組人員</td></tr>}{inventoryGroupFilter === "ALL" && <tr className="inventory-company-total"><td colSpan={5}>全公司總計</td><td>{numberText(monthlyGrandTotal)}件</td></tr>}</tbody></table>
    ...
    <div className="inventory-panel stock-inventory-panel" id="stock-inventory-image">
      <div className="inventory-panel-title"><div><h3>目前人員總件數表　總件數 :{numberText(stockGrandTotal)}件</h3>{groupFilterButtons}</div><button onClick={() => exportInventoryCanvas("stock")}>產圖</button></div>
      <table><thead><tr><th rowSpan={2}>組別</th><th rowSpan={2}>身分</th><th rowSpan={2}>人員</th><th colSpan={2}>專約</th><th colSpan={2}>一般約</th><th rowSpan={2}>租件</th><th rowSpan={2}>總件數</th></tr><tr><th>房屋</th><th>土地</th><th>房屋</th><th>土地</th></tr></thead>
      <tbody>{visibleGroupSections.map(section => { const rows = section.members.map(member => ({ member, row: stockRows.find(row => row.name === member.name) || emptyStockRow(member.name) })); const subtotal = addCategories(rows.map(item => item.row)); return <Fragment key={`stock-${section.id}`}><tr className={`inventory-group-heading group-${String(section.id).toLowerCase()}`}><td colSpan={9}><b>{section.name}</b><span>{section.members.length} 人</span></td></tr>{rows.map(({ member, row }) => <tr key={`${section.id}-${row.name}`}><td><span className={`inventory-group-badge group-${String(section.id).toLowerCase()}`}>{section.name}</span></td><td>{member.role === "組長" ? <strong className="inventory-leader-badge">組長</strong> : <span className="inventory-member-label">組員</span>}</td><td>{row.name}</td><td>{numberText(row.exclusiveHouse)}</td><td>{numberText(row.exclusiveLand)}</td><td>{numberText(row.generalHouse)}</td><td>{numberText(row.generalLand)}</td><td>{numberText(row.rental)}</td><td className="inventory-total"><button className="inventory-count-button" onClick={() => setStockDetailName(row.name)}>{numberText(row.total)}</button></td></tr>)}<tr className="inventory-group-subtotal"><td colSpan={3}>{section.name}小計</td><td>{numberText(subtotal.exclusiveHouse)}</td><td>{numberText(subtotal.exclusiveLand)}</td><td>{numberText(subtotal.generalHouse)}</td><td>{numberText(subtotal.generalLand)}</td><td>{numberText(subtotal.rental)}</td><td>{numberText(subtotal.total)}</td></tr></Fragment>; })}{!visibleGroupSections.some(section => section.members.length) && <tr><td colSpan={9} className="inventory-empty">目前沒有分組人員</td></tr>}{inventoryGroupFilter === "ALL" && <tr className="inventory-company-total"><td colSpan={8}>全公司總計</td><td>{numberText(stockGrandTotal)}</td></tr>}</tbody></table>
    </div></div>
