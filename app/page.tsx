...
<div className="topbar-row"><div className="brand"><h1>總表　管理模式 <small className="app-version">V357</small></h1></div>
  ...
  return <section className="print-document">
    <article className={`print-page data-page excel-print ${draft.propertyKind === "純土地" ? "land-print" : "house-print"}`}>
      ...
      <div className="print-main"><div className="print-left">
        ...
        <PrintRightRow label="物件編號" value={val("委託主約編號")}/><PrintRightRow label="鑰匙位置" value={val("鑰匙位置")}/><PrintRightRow label="物件現況" value={val("(物件)現況")}/><PrintRightRow label="國小" value={val("鄰近國小")}/><PrintRightRow label="國中" value={val("鄰近國中")}/><PrintRightRow label="高中" value={val("鄰近高中")}/><PrintRightRow label="大專" value={val("鄰近大專")}/><PrintRightRow label="市場" value={val("市場/購物")}/><PrintRightRow label="公園" value={val("公園綠地")}/><hr/>{draft.propertyKind === "房屋" && <PrintRightRow label="格局圖" value={val("進案文件 [格局圖]")}/>}<PrintRightRow label="現詢調" value={val("進案文件 [現詢調]")}/><PrintRightRow label="智能照片" value={val("進案文件 [物件照片 上傳系統]")}/>{draft.propertyKind === "房屋" && <><PrintRightRow label="智能主約" value={val("進案文件 [主約 契約拍照 上傳系統]")}/><PrintRightRow label="智能契變" value={val("進案文件 [契變 照片上傳系統]")}/></>}<div className="attention"><b>帶看注意事項：</b><span>中人：{val("中人(介紹費)")}</span><p>{val("注意事項")}</p></div>
      </div></div>
