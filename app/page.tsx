...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (coordinateHelp) coordinateHelp.textContent = "請貼上「緯度, 經度」；沒有正確座標不能下載 Excel。此欄只套用本次下載。";
...
<div className="topbar-row"><div className="brand"><h1>總表　管理模式 <small className="app-version">V443</small></h1></div>
    ...
    {printEditor && <div className="modal-backdrop print-editor-backdrop"><div className="modal print-editor-modal compact">
      ...
      <div className="print-editor-body compact-body">
        <section className="print-editor-section basic"><div className="print-editor-compact-grid">
          ...
          <label className="field print-editor-locationMapInput"><span>{(typeShort(printEditor.data.type) === "土地" || /^(?:LG|LA)/i.test(printEditor.data.propertyNo || "")) ? "土地位置座標（選填）" : "位置圖定位（選填）"}</span><input type="text" value={printEditor.data.locationMapInput || ""} onChange={event => setPrintEditor({ ...printEditor, data: { ...printEditor.data, locationMapInput: event.target.value } })} placeholder={(typeShort(printEditor.data.type) === "土地" || /^(?:LG|LA)/i.test(printEditor.data.propertyNo || "")) ? "例如：22.97354854089914, 120.2238506558218" : "可貼完整地址，或 22.97354854089914, 120.2238506558218"}/><small>{(typeShort(printEditor.data.type) === "土地" || /^(?:LG|LA)/i.test(printEditor.data.propertyNo || "")) ? "請貼上緯度, 經度；多筆地號擇一筆即可。此欄只套用本次下載。" : "留白會使用上方地址；經緯度最準確。此欄只套用本次下載，不會修改案件地址。"}</small></label>
