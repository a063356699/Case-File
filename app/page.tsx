...
<div className="topbar-row"><div className="brand"><h1>總表　管理模式 <small className="app-version">V378</small></h1></div>
...
function KeySummary({ records }: { records: RecordItem[] }) {
  ...
  const cells = (number: number) => { const grouped = byNumber.get(number) || []; const primary = grouped[0]; const sameAddress = grouped.length > 0 && grouped.every(record => String(record.address || "").trim() === String(primary?.address || "").trim()); return <><td className="key-number">{number}</td><td>{sameAddress ? actualDistrict(primary) : <span className="key-two-lines">{grouped.map(record => <span key={record.id}>{actualDistrict(record)}</span>)}</span>}</td><td className="key-case">{grouped.map(record => <span key={record.id} className="key-two-lines key-case-lines"><span>{record.propertyNo || ""}</span>{chunkText(record.caseName || "", 12).map((line, index) => <span key={`${record.id}-${line}-${index}`}>{line}</span>)}{record.archived && <small style={{ color: "#d71920", fontSize: "calc(.68em + 1.5pt)", fontWeight: 400, display: "block", whiteSpace: "nowrap" }}>{`${shortRocMonthDay(record._archiveActionDate || record.archived)}${archiveStatusOf(record)}`}</small>}</span>)}</td><td className="key-address">{sameAddress ? twoLineText(primary?.address || "") : <span className="key-two-lines">{grouped.map(record => <span key={record.id}>{record.address || ""}</span>)}</span>}</td><td><span className="key-two-lines">{grouped.flatMap(record => developerNameLines(record.developer || "")).map((name, index) => <span key={`${name}-${index}`}>{name}</span>)}</span></td></>; };
...
function DailyActivity({ records, compact = false, onEdit }: { records: RecordItem[]; compact?: boolean; onEdit?: (record: RecordItem) => void }) {
  ...
  const removedRecords = records.filter(record => !!record.archived && dateOnly(record._archiveActionDate || record.archived) === selectedDate).map(record => ({ ...record, caseNameNote: `${shortRocMonthDay(record._archiveActionDate || record.archived)}${archiveStatusOf(record)}`, _dailyHighlight: "[]" }));
  const groups = [
