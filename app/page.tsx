...
export default function Home() {
  ...
  return <main lang="zh-Hant-TW" className={internalView ? `internal-public-app${publicAuthReady ? " public-auth-ready" : ""}` : ""}>
    {!internalView && <header className="topbar">
        <div className="topbar-row"><div className="brand"><h1>總表　管理模式 <small className="app-version">V463</small></h1></div>
...
function DailyActivity({ records, compact = false, onEdit }: { records: RecordItem[]; compact?: boolean; onEdit?: (record: RecordItem) => void }) {
  ...
  const removedRecords = records.filter(record => !isCommissioned(record) && dateOnly(record.archived || record._archiveActionDate) === selectedDate).map(record => {
