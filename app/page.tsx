...
export default function Home() {
  ...
  const pptExtraCandidates = [...active.filter(record => !belongsToPptWeek(record, selectedPptWeek)), ...pptDraftRecords];
  const selectedPptRecords = sortPptRecords([...records.filter(record => weeklyPptRecords.some(item => item.id === record.id) || pptExtraIds.includes(record.id)), ...pptDraftRecords.filter(record => pptExtraIds.includes(record.id))]);
  ...
  return <main lang="en-GB" className={internalView ? "internal-public-app" : ""}>
    {!internalView && <header className="topbar">
      <div className="brand"><h1>總表　管理模式 <small className="app-version">V33</small></h1></div>
...
function DailyActivity({ records, compact = false, onEdit }: { records: RecordItem[]; compact?: boolean; onEdit?: (record: RecordItem) => void }) {
  ...
  return <section className={`daily-activity${compact ? " compact" : ""}`} onClick={event => { if (!onEdit) return; const cell = (event.target as HTMLElement).closest("td.col-caseName"); if (!cell) return; const article = cell.closest("article.daily-card"); const row = cell.closest("tbody tr:not(.area-group-row)"); if (!article || !row) return; const groupIndex = Array.from(event.currentTarget.querySelectorAll("article.daily-card")).indexOf(article); const rowIndex = Array.from(article.querySelectorAll("tbody tr:not(.area-group-row)")).indexOf(row); const record = sortActiveRecords(groups[groupIndex]?.records || [])[rowIndex]; if (record) onEdit(record); }}>
    <div className="daily-activity-head">
      ...
      <div className="daily-date"><span>顯示</span><button type="button" className={selectedDate === yesterday ? "selected" : ""} onClick={() => setSelectedDate(yesterday)}>昨天</button><button type="button" className={selectedDate === today() ? "selected" : ""} onClick={() => setSelectedDate(today())}>今天</button><input aria-label="選擇每日物件動態日期" type="date" value={selectedDate} onChange={event => setSelectedDate(event.target.value)}/><b>{displayRocDate(selectedDate)}</b></div>
    </div>
