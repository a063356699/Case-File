...
export default function Home() {
  ...
  return <main lang="en-GB" className={internalView ? `internal-public-app${publicAuthReady ? " public-auth-ready" : ""}` : ""}>
    {!internalView && <header className="topbar">
      <div className="topbar-row"><div className="brand"><h1>總表　管理模式 <small className="app-version">V281</small></h1></div>
...
function BusinessInventory({ records, settings, setSettings }: { records: RecordItem[]; settings: Settings; setSettings: (settings: Settings) => void }) {
  ...
  const visibleGroupSections = inventoryGroupFilter === "ALL" ? groupSections : groupSections.filter(section => section.id === inventoryGroupFilter);
  ...
  return <section className="content business-inventory-page">
    ...
    <div className="inventory-panel combined-inventory-panel" id="combined-inventory-image">
      ...
      <table><colgroup><col className="identity-col"/><col className="identity-col"/><col className="person-col"/><col className="monthly-stat-col"/><col className="monthly-stat-col"/><col className="monthly-stat-col"/><col className="stock-stat-col"/><col className="stock-stat-col"/><col className="stock-stat-col"/><col className="stock-stat-col"/><col className="stock-stat-col"/><col className="stock-stat-col"/></colgroup><thead><tr><th rowSpan={2}>組別</th><th rowSpan={2}>身分</th><th rowSpan={2}>人員</th><th className="monthly-stat-head" colSpan={3}>{rocMonth}進案統計</th><th className="stock-stat-head" colSpan={6}>目前庫存</th></tr><tr><th className="monthly-stat-subhead">本月進案</th><th className="monthly-stat-subhead">本月下架</th><th className="monthly-stat-subhead">本月件數</th><th className="stock-stat-subhead">房專</th><th className="stock-stat-subhead">土專</th><th className="stock-stat-subhead">房一</th><th className="stock-stat-subhead">土一</th><th className="stock-stat-subhead">租件</th><th className="stock-stat-subhead">總件數</th></tr></thead>
      <tbody>{visibleGroupSections.map(section => { const rows = section.members.map(member => ({ member, monthly: monthlyRows.find(row => row.name === member.name) || emptyMonthlyRow(member.name), stock: stockRows.find(row => row.name === member.name) || emptyStockRow(member.name) })); const entered = addCategories(rows.map(item => item.monthly.entered)); const archived = addCategories(rows.map(item => item.monthly.archived)); const monthlyTotal = addCategories(rows.map(item => item.monthly.total)); const stockTotal = addCategories(rows.map(item => item.stock)); return <Fragment key={`combined-${section.id}`}><tr className={`inventory-group-heading group-${String(section.id).toLowerCase()}`}><td colSpan={12}><b>{section.name}</b><span>{section.members.length} 人</span></td></tr>{rows.map(({ member, monthly, stock }) => <Fragment key={`${section.id}-${member.name}`}><tr><td><span className={`inventory-group-badge group-${String(section.id).toLowerCase()}`}>{section.name}</span></td><td>{member.role === "組長" ? <strong className="inventory-leader-badge">組長</strong> : <span className="inventory-member-label">組員</span>}</td><td>{member.name}</td><td className="monthly-stat-cell">{categorySummary(monthly.entered)}</td><td className="monthly-stat-cell">{categorySummary(monthly.archived)}</td><td className="monthly-stat-cell inventory-total"><button className="inventory-count-button" onClick={() => setMonthlyDetailName(member.name)}>{categorySummary(monthly.total, true)}</button></td><td className="stock-stat-cell">{numberText(stock.exclusiveHouse)}</td><td className="stock-stat-cell">{numberText(stock.exclusiveLand)}</td><td className="stock-stat-cell">{numberText(stock.generalHouse)}</td><td className="stock-stat-cell">{numberText(stock.generalLand)}</td><td className="stock-stat-cell">{numberText(stock.rental)}</td><td className="stock-stat-cell inventory-total"><button className="inventory-count-button" onClick={() => setStockDetailName(member.name)}>{numberText(stock.total)}</button></td></tr>{monthlyExpanded && monthly.total.total > 0 && <tr className="inventory-month-detail-row monthly-expanded-detail"><td colSpan={12}><b className="inline-detail-title">{member.name}　本月進案明細</b><div className="inventory-case-list inventory-inline-detail">{monthlyItemsFor(member.name).map(({ record, action, weight }, index) => <div key={`${record.id}-${action}-${index}`}><em>{action}</em>{inventoryCaseLine(record)}<strong>{numberText(weight)}件</strong></div>)}</div></td></tr>}{stockExpanded && stock.total > 0 && <tr className="inventory-month-detail-row stock-expanded-detail"><td colSpan={12}><b className="inline-detail-title">{member.name}　目前庫存明細</b><div className="inventory-case-list inventory-inline-detail">{stockItemsFor(member.name).map(({ record, weight }) => <div key={`stock-${record.id}`}><em>庫存</em>{inventoryCaseLine(record)}<strong>{numberText(weight)}件</strong></div>)}</div></td></tr>}</Fragment>)}<tr className="inventory-group-subtotal"><td colSpan={3}>{section.name}小計</td><td className="monthly-stat-cell">{categorySummary(entered)}</td><td className="monthly-stat-cell">{categorySummary(archived)}</td><td className="monthly-stat-cell">{categorySummary(monthlyTotal, true)}</td><td className="stock-stat-cell">{numberText(stockTotal.exclusiveHouse)}</td><td className="stock-stat-cell">{numberText(stockTotal.exclusiveLand)}</td><td className="stock-stat-cell">{numberText(stockTotal.generalHouse)}</td><td className="stock-stat-cell">{numberText(stockTotal.generalLand)}</td><td className="stock-stat-cell">{numberText(stockTotal.rental)}</td><td className="stock-stat-cell">{numberText(stockTotal.total)}</td></tr></Fragment>; })}{!visibleGroupSections.some(section => section.members.length) && <tr><td colSpan={12} className="inventory-empty">目前沒有分組人員</td></tr>}{inventoryGroupFilter === "ALL" && <tr className="inventory-company-total"><td colSpan={5}>全公司總計</td><td>{numberText(monthlyGrandTotal)}件</td><td colSpan={5}></td><td>{numberText(stockGrandTotal)}件</td></tr>}</tbody></table>
