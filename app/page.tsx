...
export default function Home() {
  ...
  return <main lang="en-GB" className={internalView ? `internal-public-app${publicAuthReady ? " public-auth-ready" : ""}` : ""}>
    {!internalView && <header className="topbar">
      <div className="topbar-row"><div className="brand"><h1>總表　管理模式 <small className="app-version">V279</small></h1></div>
...
function BusinessInventory({ records, settings, setSettings }: { records: RecordItem[]; settings: Settings; setSettings: (settings: Settings) => void }) {
  ...
  return <section className="content business-inventory-page">
    ...
    <div className="inventory-panel combined-inventory-panel" id="combined-inventory-image">
      <div className="inventory-panel-title"><div><h3>{rocMonth}進案與目前庫存統計</h3><p>本月進案 {numberText(monthlyGrandTotal)} 件　｜　目前庫存 {numberText(stockGrandTotal)} 件</p>{groupFilterButtons}</div><div className="inventory-title-actions"><button onClick={exportInventoryCanvas}>產圖</button><button onClick={() => setMonthlyExpanded(value => !value)}>{monthlyExpanded ? "收合明細" : "展開本月明細"}</button></div></div>
      <table><thead><tr><th rowSpan={2}>組別</th><th rowSpan={2}>身分</th><th rowSpan={2}>人員</th><th colSpan={3}>{rocMonth}進案統計</th><th colSpan={6}>目前庫存</th></tr><tr><th>本月進案</th><th>本月下架</th><th>本月件數</th><th>房專</th><th>土專</th><th>房一</th><th>土一</th><th>租件</th><th>總件數</th></tr></thead>
      <tbody>{visibleGroupSections.map(section => { const rows = section.members.map(member => ({ member, monthly: monthlyRows.find(row => row.name === member.name) || emptyMonthlyRow(member.name), stock: stockRows.find(row => row.name === member.name) || emptyStockRow(member.name) })); const entered = addCategories(rows.map(item => item.monthly.entered)); const archived = addCategories(rows.map(item => item.monthly.archived)); const monthlyTotal = addCategories(rows.map(item => item.monthly.total)); const stockTotal = addCategories(rows.map(item => item.stock)); return <Fragment key={`combined-${section.id}`}><tr className={`inventory-group-heading group-${String(section.id).toLowerCase()}`}><td colSpan={12}><b>{section.name}</b><span>{section.members.length} 人</span></td></tr>{rows.map(({ member, monthly, stock }) => <Fragment key={`${section.id}-${member.name}`}><tr><td><span className={`inventory-group-badge group-${String(section.id).toLowerCase()}`}>{section.name}</span></td><td>{member.role === "組長" ? <strong className="inventory-leader-badge">組長</strong> : <span className="inventory-member-label">組員</span>}</td><td>{member.name}</td><td>{categorySummary(monthly.entered)}</td><td>{categorySummary(monthly.archived)}</td><td className="inventory-total"><button className="inventory-count-button" onClick={() => setMonthlyDetailName(member.name)}>{categorySummary(monthly.total, true)}</button></td><td>{numberText(stock.exclusiveHouse)}</td><td>{numberText(stock.exclusiveLand)}</td><td>{numberText(stock.generalHouse)}</td><td>{numberText(stock.generalLand)}</td><td>{numberText(stock.rental)}</td><td className="inventory-total"><button className="inventory-count-button" onClick={() => setStockDetailName(member.name)}>{numberText(stock.total)}</button></td></tr>{monthlyExpanded && monthly.total.total > 0 && <tr className="inventory-month-detail-row"><td colSpan={12}><div className="inventory-case-list inventory-inline-detail">{monthlyItemsFor(member.name).map(({ record, action, weight }, index) => <div key={`${record.id}-${action}-${index}`}><em>{action}</em>{inventoryCaseLine(record)}<strong>{numberText(weight)}件</strong></div>)}</div></td></tr>}</Fragment>)}<tr className="inventory-group-subtotal"><td colSpan={3}>{section.name}小計</td><td>{categorySummary(entered)}</td><td>{categorySummary(archived)}</td><td>{categorySummary(monthlyTotal, true)}</td><td>{numberText(stockTotal.exclusiveHouse)}</td><td>{numberText(stockTotal.exclusiveLand)}</td><td>{numberText(stockTotal.generalHouse)}</td><td>{numberText(stockTotal.generalLand)}</td><td>{numberText(stockTotal.rental)}</td><td>{numberText(stockTotal.total)}</td></tr></Fragment>; })}{!visibleGroupSections.some(section => section.members.length) && <tr><td colSpan={12} className="inventory-empty">目前沒有分組人員</td></tr>}{inventoryGroupFilter === "ALL" && <tr className="inventory-company-total"><td colSpan={5}>全公司總計</td><td>{numberText(monthlyGrandTotal)}件</td><td colSpan={5}></td><td>{numberText(stockGrandTotal)}件</td></tr>}</tbody></table>
