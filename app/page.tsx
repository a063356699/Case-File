...
export default function Home() {
  ...
  const [archiveChoice, setArchiveChoice] = useState<{ record: RecordItem; status: string; date: string; salesPerson: string } | null>(null);
  ...
  const archiveRecord = (r: RecordItem, status = "下架洽開發", archiveDate = today(), salesPerson = r.salesPerson || "") => {
    ...
    setRecords(prev => prev.map(x => x.id === r.id ? { ...x, status, archived: normalizeDateInput(archiveDate) || today(), salesPerson: salesPerson.trim(), _archiveActionDate: today(), ...Object.fromEntries(cleanupKeys.map(key => [key, ""])) } : x)); setArchiveChoice(null); flash("已移至封存，並建立下架待辦");
  };
  ...
  const requestArchive = (record: RecordItem, status: string) => setArchiveChoice({ record, status, date: status === "到期下架" ? nextDate(record.entrustEnd) : today(), salesPerson: record.salesPerson || "" });
  ...
  return <main lang="en-GB" className={internalView ? "internal-public-app" : ""}>
    {!internalView && <header className="topbar">
      <div className="topbar-row"><div className="brand"><h1>總表　管理模式 <small className="app-version">V195</small></h1></div>
