...
const websiteListNote = (record: RecordItem) => ([
  ["591", "platform591"], ["YES319", "yes319"], ["HOUSE INFOR", "houseinfor"], ["櫥窗", "windowAd"], ["LED", "led"], ["我家網", "homeWeb"], ["5168", "price5168"], ["黃金曝光", "goldExposure"],
] as const).flatMap(([label, key]) => {
  const raw = String(record[key] || "").trim();
  if (!raw || /^(?:無|旁5)$/i.test(raw)) return [];
  const note = raw.replace(/[0-9０-９A-Za-zＡ-Ｚａ-ｚ]/g, "").replace(/[\s－—–_\-／/、,，.:：#＃()（）\[\]【】]/g, "").replace(label, "");
  return /[\u4e00-\u9fff]/.test(note) ? [`${label}${note}`] : [];
}).join("　");
