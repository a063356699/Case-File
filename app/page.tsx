...
export default function Home() {
  ...
  const markIntakeDraftPrintedForSales = (id: string) => {
    ...
    if (!target) return;
    const printedForSalesAt = new Date().toISOString();
    setIntakeDrafts(previous => previous.map(draft => draft.id === id ? { ...draft, printedForSalesAt } : draft));
    flash(`已記錄「${intakeValue(target.values, "案名") || "未命名案件"}」列印給業務`);
  };
  ...
  return <main lang="en-GB" className={internalView ? "internal-public-app" : ""}>
    {!internalView && <header className="topbar">
      <div className="brand"><h1>總表　管理模式 <small className="app-version">V6</small></h1></div>
...
<div className="modal-head-actions">{records.some(record => record.id === editing.id) && !editing.archived && (editing.status || "委託中") === "委託中" && <><button className="record-print-button color" type="button" onClick={() => { const attention = colorSheetAttention(editing.attentionNotes || "", editing.additionNotes || ""); setPrintEditor({ kind: "color", data: { ...editing, notes: attention, attentionNotes: attention, photos: [...(editing.photos || [])] } }); }}>下載彩色表 Excel</button><button className={`record-print-button issue${editing.colorSheetIssue ? " has-issue" : ""}`} type="button" onClick={() => { const attention = colorSheetAttention(editing.attentionNotes || "", editing.additionNotes || ""); setPrintEditor({ kind: "color", data: { ...editing, notes: attention, attentionNotes: attention, photos: [...(editing.photos || [])] } }); }}>{editing.colorSheetIssue ? "彩色表缺件紀錄 ●" : "彩色表缺件紀錄"}</button><button className="record-print-button cover" type="button" onClick={() => printRecordDocument(editing, "cover")}>列印新進封面</button></>}<button className="close" onClick={requestCloseEditing}>×</button></div></div>
...
function IntakePanel({ raw, setRaw, drafts, draft, selectDraft, deleteDraft, analyze, addManualDraft, updateValue, clear, confirmIntake, markPrintedForSales }: { raw: string; setRaw: (value: string) => void; drafts: IntakeData[]; draft: IntakeData | null; selectDraft: (id: string) => void; deleteDraft: (id: string) => void; analyze: () => void; addManualDraft: () => void; updateValue: (key: string, value: string) => void; clear: () => void; confirmIntake: (id?: string) => void; markPrintedForSales: (id: string) => void }) {
  ...
  const draftCard = (item: IntakeData) => <article className={`draft-card ${draft?.id === item.id ? "selected" : ""}`} key={item.id}>
    ...
    <div className="draft-actions"><button onClick={() => printDraft(item.id)}>列印</button><button className={item.printedForSalesAt ? "draft-printed-recorded" : ""} onClick={() => !item.printedForSalesAt && markPrintedForSales(item.id)} disabled={!!item.printedForSalesAt}>{item.printedForSalesAt ? `已列印給業務 ${displayRocDate(item.printedForSalesAt)}` : "已列印給業務"}</button><button className="primary" disabled={!!item.linkedRecordId} onClick={() => confirmIntake(item.id)}>{item.linkedRecordId ? "已進案" : "進案"}</button><button className="danger" onClick={() => deleteDraft(item.id)}>刪除</button></div>
  </article>;
