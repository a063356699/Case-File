...
export default function Home() {
  ...
  return <main lang="en-GB" className={internalView ? "internal-public-app" : ""}>
    {!internalView && <header className="topbar">
      <div className="topbar-row"><div className="brand"><h1>總表　管理模式 <small className="app-version">V176</small></h1></div>
...
function TourPlanner({ records, drafts, items, setItems, editRecord, updateDraftCaseName, tourDate, setTourDate, tourTitle, setTourTitle, notify, complete }: { records: RecordItem[]; drafts: IntakeData[]; items: TourItem[]; setItems: (items: TourItem[]) => void; editRecord: (record: RecordItem) => void; updateDraftCaseName: (id: string, caseName: string) => void; tourDate: string; setTourDate: (date: string) => void; tourTitle: string; setTourTitle: (title: string) => void; notify: (text: string) => void; complete: (date: string, recordIds: string[], draftIds: string[]) => void }) {
  ...
  const exportTourImage = () => {
    ...
    rows.forEach((row, rowIndex) => { x = margin; row.forEach((text, index) => { if (index === 4) drawSizedCell(text || "—", x, y, widths[index], rowHeights[rowIndex], 25, true, true); else if (index === 6) drawAgeCell(text || "—", x, y, widths[index], rowHeights[rowIndex]); else if (index === 8) drawSizedCell(text || "—", x, y, widths[index], rowHeights[rowIndex], 22, false, true); else if ([9, 10, 11].includes(index)) drawSizedCell(text || "—", x, y, widths[index], rowHeights[rowIndex], 24, false, true); else if (index === 17) drawFrontageDepthCell(text, x, y, widths[index], rowHeights[rowIndex]); else if (index === 20) drawSizedCell(text || "—", x, y, widths[index], rowHeights[rowIndex], 24, true); else if (index === 22) drawSingleLineCell(text || "—", x, y, widths[index], rowHeights[rowIndex], false, 24); else drawCell(text || "—", x, y, widths[index], rowHeights[rowIndex], index === 2); x += widths[index]; }); y += rowHeights[rowIndex]; });
    try {
