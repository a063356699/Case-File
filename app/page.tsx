...
const fields = [
  ...
  ["authorizationStatus", "授權書狀態"], ["authorizationCopyType", "授權書本別"], ["authorizationCopyCount", "授權書影本（張）"], ["authorizationOriginalCount", "授權書正本（張）"],
...
<div className="topbar-row"><div className="brand"><h1>總表　管理模式 <small className="app-version">V388</small></h1></div>
...
function printRecordDocument(record: RecordItem, kind: "color" | "cover") {
  ...
  const coverV3 = `<div class="page intake-cover-v3"><style>
    ...
    <div class="document-wrap"><div class="vertical-label document-label"><span>內</span><span>附</span><span>文</span><span>件</span><span>之</span><span>正</span><span>本</span></div><div class="document-content">
      ...
      <div class="sub">●此物件為銷售土地需要且<span class="form-box">${coverZoningStatus === "使用分區申請" ? "✓" : ""}</span>使用分區申請　日期<span class="form-box cover-date-box">${escapeHtml(record.zoningApplicationDate ? displayRocDate(record.zoningApplicationDate) : "")}</span></div>
      <div class="sub">●此物件為銷售土地需要且<span class="form-box">${["已附分區證明", "土地已附正式分區"].includes(coverZoningStatus) ? "✓" : ""}</span>已附分區證明×<span class="form-box">${escapeHtml(record.zoningCertificateCount || "")}</span>張</div>
...
function Field({ fieldKey, label, record, records, setRecord }: { fieldKey: string; label: string; record: RecordItem; records: RecordItem[]; setRecord: (r: RecordItem) => void }) {
  ...
  const pillChoices: Record<string, string[]> = {
    ...
    authorizationStatus: ["缺授", "無需要", "已附上歸檔"],
  ...
  if (fieldKey === "zoningDocumentStatus") {
    const displayedValue = value === "土地已附正式分區" ? "已附分區證明" : value;
    const options = ["房屋不需要", "使用分區申請", "已附分區證明", "謄本已標示不用附"];
    return <label className={`field cover-pill-field cover-pill-${fieldKey}`}><span>{label}</span><span className="pill-options">{options.map(option => <button type="button" className={displayedValue === option ? "active" : ""} onClick={() => set(displayedValue === option ? "" : option)} key={option}>{option}</button>)}</span>{displayedValue === "使用分區申請" && <span className="cover-detail-input"><b>申請日期</b><input type="text" inputMode="numeric" value={displayRocDate(record.zoningApplicationDate || "")} onChange={event => setRecord({ ...record, zoningDocumentStatus: "使用分區申請", zoningApplicationDate: event.target.value })} onBlur={event => setRecord({ ...record, zoningDocumentStatus: "使用分區申請", zoningApplicationDate: normalizeDateInput(event.target.value) })} placeholder="例如 115/9/3"/></span>}{displayedValue === "已附分區證明" && <span className="cover-detail-input"><b>已附分區證明</b><input type="text" inputMode="numeric" value={record.zoningCertificateCount || ""} onChange={event => setRecord({ ...record, zoningDocumentStatus: "已附分區證明", zoningCertificateCount: event.target.value.replace(/\D/g, "") })} placeholder="張數"/><i>張</i></span>}</label>;
  }
  if (fieldKey === "authorizationCopyType") return <label className="field authorization-count-field"><span>{label}</span><span className="authorization-count-fields"><b>影本</b><input type="text" inputMode="numeric" value={record.authorizationCopyCount || ""} onChange={event => setRecord({ ...record, authorizationCopyCount: event.target.value.replace(/\D/g, ""), authorizationCopyType: "" })} placeholder="張數"/><i>張</i><b>正本</b><input type="text" inputMode="numeric" value={record.authorizationOriginalCount || ""} onChange={event => setRecord({ ...record, authorizationOriginalCount: event.target.value.replace(/\D/g, ""), authorizationCopyType: "" })} placeholder="張數"/><i>張</i></span></label>;
