...
const dateKeys = new Set(["entrustStart", "entrustEnd", "reportDate", "updateDate", "groupViewDate", "showingFollowUpDueDate", "bookLocationDate", "salesBookDate", "coverChangeDate", "zoningApplicationDate", "platform591Expiry", "price5168Expiry", "goldExposureExpiry"]);
...
<div className="topbar-row"><div className="brand"><h1>總表　管理模式 <small className="app-version">V456</small></h1></div>
  ...
  return <section className="content intake-page">
    ...
    {draft && <>
      <div className="modal-backdrop intake-modal-backdrop" onMouseDown={event => event.target === event.currentTarget && selectDraft("")}><div className="modal intake-draft-modal">
        ...
        <div className="intake-edit-grid">{intakeEditFields.filter(([key]) => !isLandIntakeDraft(draft) || !landHiddenIntakeFields.has(key)).map(([key, label]) => { const rawValue = intakeValue(draft.values, key); const dateField = isEntrustDate(key); const completionDateField = isCompletionDate(key); const normalizedDate = normalizeDateInput(rawValue); const noContractChangeField = key === "進案文件 [契變 照片上傳系統]"; return <label className={`${intakeFieldClass(key)}${noContractChangeField ? " intake-contract-change-field" : ""}`} key={key}><span className={key === "特色說明1" ? "intake-feature-title" : ""}>{label}{key === "特色說明1" && <button type="button" className="copy-features-button" onClick={copyFeatures}>{featuresCopied ? "已複製" : "複製特色1～4"}</button>}</span><input inputMode={dateField || completionDateField ? "numeric" : undefined} placeholder={dateField ? "例如 115/8/6" : completionDateField ? "例如 079.11.17" : undefined} value={dateField ? displayRocDate(normalizedDate) : completionDateField ? displayRocDotDate(normalizedDate) : rawValue} onChange={e => updateValue(key, e.target.value)} onBlur={dateField || completionDateField ? e => updateValue(key, normalizeDateInput(e.target.value)) : undefined}/>{noContractChangeField && <button type="button" className={rawValue === "進案無契變" ? "intake-no-contract-change selected" : "intake-no-contract-change"} onClick={() => updateValue(key, rawValue === "進案無契變" ? "" : "進案無契變")}>進案無契變</button>}</label>; })}</div>
