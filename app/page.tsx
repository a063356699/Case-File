...
export default function Home() {
  ...
  return <main className={internalView ? "internal-public-app" : ""}>
    ...
    <section className={`content ${tab === "archive" ? "archive-list-page" : "active-list-page"}`}>
      <div className="list-head"><div className="list-title-search"><SectionTitle title={tab === "archive" ? "封存物件" : "委託中物件"} subtitle={tab === "archive" ? "到期與已下架物件紀錄" : ""}/><label className="search">⌕<input value={query} onChange={e => setQuery(e.target.value)} placeholder="搜尋編號、地區、案名、地址…"/></label></div><div className="tools">{tab === "active" && <><select className="website-filter" value={websiteFilter} onChange={event => setWebsiteFilter(event.target.value)}><option value="">全部物件</option><option value="all">全部尚未 PO</option>{Object.entries(websiteFieldMap).map(([key, name]) => <option key={key} value={key}>{name} 尚未 PO</option>)}</select><button className="advanced-filter-button" onClick={() => setAdvancedFilterOpen(true)}>進階篩選</button><button className="clear-filter-button" onClick={() => { setWebsiteFilter(""); setAdvancedFilter(blankAdvancedFilter()); setQuery(""); }}>清除篩選</button><button className={missingDataRecords.length ? "missing-data-button has-items" : "missing-data-button"} onClick={() => setMissingDataReminderOpen(true)}>待補資料 {missingDataRecords.length}</button></>}</div></div>
