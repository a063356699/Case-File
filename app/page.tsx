...
const publicColumns = ["propertyNo", "area", "caseName", "address", "type", "contractType", "status", "price", "direction", "age", "floor", "layout", "indoorPing", "buildingPing", "landPing", "parking", "managementFee", "key", "currentState", "road", "frontage", "depth", "zoning", "coverageFar", "developer", "reportDate", "updateDate", "groupViewDate", "photoInfo"];
...
export default function Home() {
  ...
  return <main>
    ...
    <section className={`content ${tab === "archive" ? "archive-list-page" : "active-list-page"}`}>
      ...
      {!shown.length && <Empty text={query ? "找不到符合的物件" : tab === "archive" ? "目前沒有封存物件" : "目前沒有委託中物件"}/>}
    </section>}
...
function CellContent({ record: r, column: k }: { record: RecordItem; column: string }) {
  ...
  if (["platform591", "price5168", "goldExposure", "yes319", "houseinfor", "homeWeb", "windowAd", "led"].includes(k)) { if (r[`${k}None`] === "1") return <>無</>; return <span className="two-line-value website-list-value"><span>{r[k] || "—"}</span>{r[`${k}DownDate`] && <small className="archive-down-note">{displayRocDate(r[`${k}DownDate`])}下架</small>}</span>; }
...
function PropertyBookReview({ records, submit }: { records: RecordItem[]; submit: (reviews: Array<{ record: RecordItem; status: string }>) => void }) {
  ...
  useEffect(() => { let stopped = false; const endpoint = location.protocol === "file:" ? "http://localhost:8765/api/mobile-qr" : "/api/mobile-qr"; const poll = async () => { try { const response = await fetch(endpoint, { cache: "no-store" }); if (!response.ok) return; const data = await response.json(); if (!stopped) (data.events || []).forEach((event: { code?: string }) => event.code && locateCode(event.code)); } catch {} }; poll(); const timer = window.setInterval(poll, 900); return () => { stopped = true; clearInterval(timer); }; }, [records.map(record => `${record.id}:${record._bookReviewAt || ""}`).join("|")]);
  ...
  return <section className="book-review-panel"><div className="book-review-head"><div><b>物件本確認</b><span>本次確認：{displayRocDate(cycleStart)}　下次確認：{displayRocDate(nextCheckDate)}</span></div><div className="book-review-actions"><strong>{dueRecords.length} 筆待確認</strong><button onClick={() => { setScanMessage(""); setScannerOpen(true); }}>掃描QR Code</button><label className="file-button">上傳QR圖片<input type="file" accept="image/*" capture="environment" onChange={event => { decodeImage(event.target.files?.[0]); event.target.value = ""; }}/></label></div></div><div className="book-review-list">{groupedDueRecords.map(group => <Fragment key={group.area}><div className="book-review-area"><span>{group.area}</span><small>{group.items.length}筆</small></div>{group.items.map(record => <div id={`book-review-${record.id}`} className="book-review-row" key={record.id}><div><b>{record.caseName || record.propertyNo}</b><small className="book-record-line"><span>{record.propertyNo}</span><span>{record.address || "未填地址"}</span></small></div><div className="book-review-side"><small className="book-current-note">{displayRocDate(record.bookLocationDate || "") || "未填日期"}　{record.bookLocationType || "未填位置"}</small><strong className="book-scan-status">待確認</strong></div></div>)}</Fragment>)}</div><div className="book-review-foot"><span>本次已掃描 {scannedIds.length} 筆</span></div>{scannerOpen && <div className="qr-scanner-backdrop"><div className="qr-scanner-modal"><div><b>連續掃描物件QR Code</b><button onClick={() => setScannerOpen(false)}>×</button></div><video ref={videoRef} playsInline muted/><strong className="qr-scan-count">已掃描 {scannedIds.length} 筆</strong><p>{scanMessage || "將QR Code放在畫面中央，掃完一件可直接刷下一件。"}</p><button className="primary qr-scan-done" onClick={() => setScannerOpen(false)}>掃描完成</button></div></div>}</section>;
}
