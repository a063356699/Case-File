...
export default function Home() {
  ...
  return <main lang="zh-Hant-TW" className={internalView ? `internal-public-app${publicAuthReady ? " public-auth-ready" : ""}` : ""}>
    {!internalView && <header className="topbar">
        <div className="topbar-row"><div className="brand"><h1>總表　管理模式 <small className="app-version">V473</small></h1></div>
...
async function printKeyLabel(record: RecordItem) {
  ...
  printDocument.open();
  printDocument.write(`<!doctype html><html lang="zh-Hant"><head><meta charset="UTF-8"><title>鑰匙標籤 ${escapeHtml(propertyNo)}</title><style>@page{size:75mm 50mm;margin:0}*{box-sizing:border-box}html,body{width:75mm;margin:0;padding:0;background:#fff;color:#000;font-family:"Microsoft JhengHei",sans-serif;-webkit-print-color-adjust:exact;print-color-adjust:exact}.label{width:75mm;height:50mm;border:.35mm solid #000;overflow:hidden}.info{height:25mm;padding:.8mm 1.4mm .6mm;border-bottom:.3mm dashed #555;display:flex;flex-direction:column;justify-content:space-between}.topline,.case-line,.metric-line{display:flex;align-items:center;gap:1.2mm;white-space:nowrap}.topline{font-size:10pt;font-weight:900}.topline b{font-size:11pt}.topline .key{margin-left:auto}.case-line{font-size:10.5pt;font-weight:900}.case-line .case{max-width:52mm;overflow:hidden;text-overflow:ellipsis}.case-line .developer{margin-left:auto;font-size:9pt}.address{font-size:9.5pt;font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.metric-line{font-size:8.5pt;font-weight:700}.company{font-size:8pt;font-weight:700;display:flex;justify-content:space-between;align-items:center;white-space:nowrap}.property-block{height:25mm;display:grid;grid-template-rows:1fr 1fr}.property-row{padding:.8mm 1.4mm;display:flex;align-items:center;justify-content:center;text-align:center;font-weight:900;line-height:1.08;overflow:hidden;overflow-wrap:anywhere}.property-case{font-size:14pt;border-bottom:.3mm dashed #555}.property-address{font-size:12.5pt}</style></head><body><section class="label"><div class="info"><div class="topline"><b>${escapeHtml(propertyNo)}</b><span class="key">公司#${escapeHtml(keyNumber)}</span></div><div class="case-line"><span class="case">${escapeHtml(record.caseName || "—")}</span><span class="developer">${escapeHtml(developerFullNameText(record.developer) || "—")}</span></div><div class="address">${escapeHtml(record.address || "—")}</div><div class="metric-line">${metrics}</div><div class="company"><span>台南文化崇明加盟店</span><span>電話：06-3356699</span></div></div><div class="property-block"><div class="property-row property-case">${escapeHtml(record.caseName || "—")}</div><div class="property-row property-address">${escapeHtml(record.address || "—")}</div></div></section></body></html>`);
  printDocument.close();
