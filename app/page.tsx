...
export default function Home() {
  ...
  return <main lang="zh-Hant-TW" className={internalView ? `internal-public-app${publicAuthReady ? " public-auth-ready" : ""}` : ""}>
    {!internalView && <header className="topbar">
        <div className="topbar-row"><div className="brand"><h1>總表　管理模式 <small className="app-version">V471</small></h1></div>
...
async function printKeyLabel(record: RecordItem) {
  ...
  let qrDataUrl = "";
  try { qrDataUrl = await createColorWorkbookQr(keyCode); }
  catch (error) { return alert(error instanceof Error ? error.message : "QR Code 產生失敗，請先核對物件編號與公司鑰匙號碼。"); }
  try {
    const qrImage = new Image();
    qrImage.src = qrDataUrl;
    await qrImage.decode();
    ...
    qrCanvas.height = qrImage.naturalHeight;
    const qrContext = qrCanvas.getContext("2d", { willReadFrequently: true });
    if (!qrContext) throw new Error("無法建立解碼畫布");
    qrContext.drawImage(qrImage, 0, 0);
    const qrPixels = qrContext.getImageData(0, 0, qrCanvas.width, qrCanvas.height);
    const decodedKeyCode = jsQR(qrPixels.data, qrCanvas.width, qrCanvas.height, { inversionAttempts: "attemptBoth" })?.data || "";
    if (decodedKeyCode !== keyCode) throw new Error(`預期「${keyCode}」，實際「${decodedKeyCode || "無法辨識"}」`);
  } catch (error) {
    return alert(`鑰匙 QR Code 驗證失敗：${error instanceof Error ? error.message : "內容不一致"}，已停止列印。`);
  }
  const kind = typeShort(record.type || "");
  ...
  const barcodeSvg = code39Svg(keyCode);
  const frame = document.createElement("iframe");
  ...
  printDocument.open();
  printDocument.write(`<!doctype html><html lang="zh-Hant"><head><meta charset="UTF-8"><title>鑰匙標籤 ${escapeHtml(propertyNo)}</title><style>@page{size:75mm 50mm;margin:0}*{box-sizing:border-box}html,body{width:75mm;margin:0;padding:0;background:#fff;color:#000;font-family:"Microsoft JhengHei",sans-serif;-webkit-print-color-adjust:exact;print-color-adjust:exact}.label{width:75mm;height:50mm;border:.35mm solid #000;overflow:hidden;break-after:page;page-break-after:always}.label:last-child{break-after:auto;page-break-after:auto}.info{height:25mm;padding:1.2mm 1.6mm .8mm;border-bottom:.3mm dashed #555;display:flex;flex-direction:column;justify-content:space-between}.topline,.case-line,.metric-line{display:flex;align-items:center;gap:1.5mm;white-space:nowrap}.topline{font-size:8pt;font-weight:800}.topline b{font-size:9pt}.topline .key{margin-left:auto}.case-line{font-size:9pt;font-weight:900}.case-line .case{max-width:50mm;overflow:hidden;text-overflow:ellipsis}.case-line .developer{margin-left:auto;font-size:7.5pt}.address{font-size:7.4pt;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.metric-line{font-size:7.2pt}.company{font-size:6.7pt;display:flex;justify-content:space-between;align-items:center;white-space:nowrap}.codes{height:25mm;padding:1mm 1.5mm;display:grid;grid-template-columns:1fr 18mm;gap:1.5mm;align-items:center}.barcode{height:15mm;display:flex;flex-direction:column;align-items:center;justify-content:center;min-width:0}.barcode svg{width:100%;height:11.5mm}.barcode span{font-size:7.5pt;font-weight:800;letter-spacing:.08em}.qr{width:17mm;height:17mm;object-fit:contain}.property-sheet{border:0;display:flex;align-items:flex-start}.mini-stack{width:30mm;height:50mm}.mini-card{width:30mm;height:25mm;border:.35mm solid #000;padding:1.4mm;display:flex;flex-direction:column;justify-content:center;gap:1.2mm;overflow:hidden}.mini-card .card-case{font-size:6.4pt;line-height:1.25;font-weight:900;overflow-wrap:anywhere}.mini-card .card-address{padding-top:1mm;border-top:.25mm dashed #555;font-size:6pt;line-height:1.25;font-weight:800;overflow-wrap:anywhere}</style></head><body><section class="label"><div class="info"><div class="topline"><b>${escapeHtml(propertyNo)}</b><span class="key">公司#${escapeHtml(keyNumber)}</span></div><div class="case-line"><span class="case">${escapeHtml(record.caseName || "—")}</span><span class="developer">${escapeHtml(developerFullNameText(record.developer) || "—")}</span></div><div class="address">${escapeHtml(record.address || "—")}</div><div class="metric-line">${metrics}</div><div class="company"><span>台南文化崇明加盟店</span><span>電話：06-3356699</span></div></div><div class="codes"><div class="barcode">${barcodeSvg}<span>${escapeHtml(keyCode)}</span></div><img class="qr" src="${qrDataUrl}" alt="QR Code"></div></section><section class="label property-sheet"><div class="mini-stack"><div class="mini-card"><div class="card-case">${escapeHtml(record.caseName || "—")}</div><div class="card-address">${escapeHtml(record.address || "—")}</div></div><div class="mini-card"><div class="card-case">${escapeHtml(record.caseName || "—")}</div><div class="card-address">${escapeHtml(record.address || "—")}</div></div></div></section></body></html>`);
  printDocument.close();
