...
export default function Home() {
  ...
  return <main lang="en-GB" className={internalView ? "internal-public-app" : ""}>
    {!internalView && <header className="topbar">
      <div className="topbar-row"><div className="brand"><h1>總表　管理模式 <small className="app-version">V177</small></h1></div>
...
function TourPlanner({ records, drafts, items, setItems, editRecord, updateDraftCaseName, tourDate, setTourDate, tourTitle, setTourTitle, notify, complete }: { records: RecordItem[]; drafts: IntakeData[]; items: TourItem[]; setItems: (items: TourItem[]) => void; editRecord: (record: RecordItem) => void; updateDraftCaseName: (id: string, caseName: string) => void; tourDate: string; setTourDate: (date: string) => void; tourTitle: string; setTourTitle: (title: string) => void; notify: (text: string) => void; complete: (date: string, recordIds: string[], draftIds: string[]) => void }) {
  ...
  const exportTourImage = () => {
    ...
    const headers = ["序", "地區\n種類", "案名", "地址", "總價", "朝向", "屋齡", "樓層", "格局", "室內坪", "建坪", "地坪", "車位", "管理費", "鑰匙", "現況", "臨路", "面寬\n深度", "使用分區", "建蔽\n容積", "開發", "備註欄", "進案報件日期"];
      const widths = [52, 78, 170, 220, 90, 68, 120, 110, 82, 108, 105, 105, 120, 126, 112, 100, 88, 138, 105, 119, 105, 340, 143];
    ...
    let y = margin + titleHeight, x = margin; context.fillStyle = "#dcebe5"; context.fillRect(margin, y, tableWidth, headerHeight); headers.forEach((text, index) => { if (index === 22) drawSingleLineCell(text, x, y, widths[index], headerHeight, true, 21); else drawCell(text, x, y, widths[index], headerHeight, true); x += widths[index]; }); y += headerHeight;
    rows.forEach((row, rowIndex) => { x = margin; row.forEach((text, index) => { if (index === 4) drawSizedCell(text || "—", x, y, widths[index], rowHeights[rowIndex], 25, true); else if (index === 6) drawAgeCell(text || "—", x, y, widths[index], rowHeights[rowIndex]); else if (index === 8) drawSizedCell(text || "—", x, y, widths[index], rowHeights[rowIndex], 22, false, true); else if ([9, 10, 11].includes(index)) drawSizedCell(text || "—", x, y, widths[index], rowHeights[rowIndex], 24, false, true); else if (index === 17) drawFrontageDepthCell(text, x, y, widths[index], rowHeights[rowIndex]); else if (index === 20) drawSizedCell(text || "—", x, y, widths[index], rowHeights[rowIndex], 24, true); else if (index === 21) drawNotesCell(text || "—", x, y, widths[index], rowHeights[rowIndex]); else if (index === 22) drawSingleLineCell(text || "—", x, y, widths[index], rowHeights[rowIndex], false, 24); else drawCell(text || "—", x, y, widths[index], rowHeights[rowIndex], index === 2); x += widths[index]; }); y += rowHeights[rowIndex]; });
    try {
