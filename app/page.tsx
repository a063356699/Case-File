...
export default function Home() {
  ...
  useEffect(() => {
    if (!storageReady || editing || newCaseReminder || !reminderCandidate || deferredNewCaseReminderIds.current.has(reminderCandidate.id)) return;
    setNewCaseReminder({ ...reminderCandidate });
  }, [storageReady, editing, newCaseReminder, reminderCandidate]);
  ...
  const saveNewCaseReminder = () => {
    ...
    deferredNewCaseReminderIds.current.add(newCaseReminder.id);
    setNewCaseReminder(null);
    flash("新案件完成提醒已儲存");
  };
  const deferNewCaseReminder = () => {
    if (newCaseReminder) deferredNewCaseReminderIds.current.add(newCaseReminder.id);
    setNewCaseReminder(null);
  };
  ...
  return <main lang="en-GB" className={internalView ? "internal-public-app" : ""}>
    {!internalView && <header className="topbar">
      <div className="brand"><h1>總表　管理模式 <small className="app-version">V91</small></h1></div>
