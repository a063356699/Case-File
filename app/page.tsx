...
<div className="topbar-row"><div className="brand"><h1>總表　管理模式 <small className="app-version">V389</small></h1></div>
...
function Field({ fieldKey, label, record, records, setRecord }: { fieldKey: string; label: string; record: RecordItem; records: RecordItem[]; setRecord: (r: RecordItem) => void }) {
  ...
  if (fieldKey === "zoningDocumentStatus") { const displayedValue = value === "土地已附正式分區" ? "已附分區證明" : value; const options = ["房屋不需要", "使用分區申請", "已附分區證明", "謄本已標示不用附"]; return <label className={`field cover-pill-field cover-pill-${fieldKey}`}><span>{label}</span><span className="pill-options">{options.map(option => <button type="button" className={displayedValue === option ? "active" : ""} onClick={() => set(displayedValue === option ? "" : option)} key={option}>{option}</button>)}</span></label>; }
